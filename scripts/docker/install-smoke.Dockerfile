...
#   docker build -f scripts/docker/install-smoke.Dockerfile \
#     --build-arg CLAWFORGE_SMOKE_FULL=1 .
...
ARG CLAWFORGE_SMOKE_FULL=0
RUN if [ "$CLAWFORGE_SMOKE_FULL" = "1" ]; then \
        bash /tmp/install.sh; \
    else \
        # 1. Syntax check
...
RUN if [ "$CLAWFORGE_SMOKE_FULL" = "1" ] && [ -f "$HOME/.clawforge/bin/clawforge" ]; then \
        $HOME/.clawforge/bin/clawforge --version && \
        echo "PASS: clawforge binary works"; \
    else \
