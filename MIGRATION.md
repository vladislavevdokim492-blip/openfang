# Migrating to ClawForge

This guide covers migrating from OpenClaw (and other frameworks) to ClawForge. The migration engine handles config conversion, agent import, memory transfer, channel re-configuration, and skill scanning.
...
This auto-detects your OpenClaw workspace at `~/.openclaw/` and imports everything into `~/.clawforge/`.

### Options

```bash
# Specify a custom source directory
clawforge migrate --from openclaw --source-dir /path/to/openclaw/workspace
...
After a successful migration, a `migration_report.md` file is saved to `~/.clawforge/` with a summary of everything that was imported, skipped, or needs manual attention.
...
```bash
clawforge migrate --from langchain   # Coming soon
clawforge migrate --from autogpt     # Coming soon
```
...
| Item | Source (OpenClaw) | Destination (ClawForge) | Status |
|------|-------------------|------------------------|--------|
| **Config** | `~/.openclaw/config.yaml` | `~/.clawforge/config.toml` | Fully automated |
| **Agents** | `~/.openclaw/agents/*/agent.yaml` | `~/.clawforge/agents/*/agent.toml` | Fully automated |
| **Memory** | `~/.openclaw/agents/*/MEMORY.md` | `~/.clawforge/agents/*/imported_memory.md` | Fully automated |
| **Channels** | `~/.openclaw/messaging/*.yaml` | `~/.clawforge/channels_import.toml` | Automated (manual merge) |
...
Channel configurations (Telegram, Discord, Slack) are exported to a `channels_import.toml` file. You must manually merge the `[channels]` section into your `~/.clawforge/config.toml`.
...
```bash
clawforge skill install <skill-name-or-path>
```

ClawForge automatically detects OpenClaw-format skills and converts them during installation.
...
### 1. Initialize ClawForge

```bash
clawforge init
```

This creates `~/.clawforge/` with a default `config.toml`.
...
**ClawForge** (`~/.clawforge/config.toml`):
```toml
[default_model]
provider = "anthropic"
model = "claude-sonnet-4-20250514"
...
**ClawForge** (`~/.clawforge/agents/coder/agent.toml`):
```toml
name = "coder"
version = "0.1.0"
description = "A coding assistant"
...
**ClawForge** (add to `~/.clawforge/config.toml`):
```toml
[channels.telegram]
bot_token_env = "TELEGRAM_BOT_TOKEN"
default_agent = "coder"
...
Copy any `MEMORY.md` files from OpenClaw agents to ClawForge agent directories:

```bash
cp ~/.openclaw/agents/coder/MEMORY.md ~/.clawforge/agents/coder/imported_memory.md
```
...
| Aspect | OpenClaw | ClawForge |
|--------|----------|----------|
| Format | YAML | TOML |
| Config location | `~/.openclaw/config.yaml` | `~/.clawforge/config.toml` |
...
Tools were renamed between OpenClaw and ClawForge for consistency. The migration engine handles this automatically.

| OpenClaw Tool | ClawForge Tool | Notes |
|---------------|---------------|-------|
...
| OpenClaw Profile | ClawForge Tools |
|------------------|----------------|
...
| OpenClaw Name | ClawForge Name | API Key Env Var |
|---------------|---------------|-----------------|
...
| **Networking** | None | OFP (ClawForge Protocol) peer-to-peer |
...
```bash
clawforge migrate --from openclaw --source-dir /path/to/your/workspace
```
...
```bash
clawforge skill install /path/to/openclaw/skills/my-skill
```
...
```bash
cat ~/.clawforge/channels_import.toml
# Copy the [channels.*] sections into ~/.clawforge/config.toml
```
