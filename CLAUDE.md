# ClawForge — Agent Instructions

## Project Overview
ClawForge is an open-source Agent Operating System written in Rust (14 crates).
- Config: `~/.clawforge/config.toml`
- Default API: `http://127.0.0.1:4200`
- CLI binary: `target/release/clawforge.exe` (or `target/debug/clawforge.exe`)
...
```bash
GROQ_API_KEY=<key> target/release/clawforge.exe start &
...
## Architecture Notes
- **Don't touch `clawforge-cli`** — user is actively building the interactive CLI
...
## Common Gotchas
- `clawforge.exe` may be locked if daemon is running — use `--lib` flag or kill daemon first
