...
Thank you for your interest in contributing to ClawForge. This guide covers everything you need to get started, from setting up your development environment to submitting pull requests.
...
```bash
cargo test -p clawforge-kernel
cargo test -p clawforge-runtime
cargo test -p clawforge-memory
```
...
- **Naming**:
  - Types: `PascalCase` (e.g., `ClawForgeKernel`, `AgentManifest`)
  - Functions/methods: `snake_case`
  - Constants: `SCREAMING_SNAKE_CASE`
  - Crate names: `clawforge-{name}` (kebab-case)
...
ClawForge is organized as a Cargo workspace with 14 crates:

| Crate | Role |
|-------|------|
| `clawforge-types` | Shared type definitions, taint tracking, manifest signing (Ed25519), model catalog, MCP/A2A config types |
| `clawforge-memory` | SQLite-backed memory substrate with vector embeddings, usage tracking, canonical sessions, JSONL mirroring |
| `clawforge-runtime` | Agent loop, 3 LLM drivers (Anthropic/Gemini/OpenAI-compat), 38 built-in tools, WASM sandbox, MCP client/server, A2A protocol |
| `clawforge-hands` | Hands system (curated autonomous capability packages), 7 bundled hands |
| `clawforge-extensions` | Integration registry (25 bundled MCP templates), AES-256-GCM credential vault, OAuth2 PKCE |
| `clawforge-kernel` | Assembles all subsystems: workflow engine, RBAC auth, heartbeat monitor, cron scheduler, config hot-reload |
| `clawforge-api` | REST/WS/SSE API (Axum 0.8), 76 endpoints, 14-page SPA dashboard, OpenAI-compatible `/v1/chat/completions` |
| `clawforge-channels` | 40 channel adapters (Telegram, Discord, Slack, WhatsApp, and 36 more), formatter, rate limiter |
| `clawforge-wire` | OFP (ClawForge Protocol): TCP P2P networking with HMAC-SHA256 mutual authentication |
| `clawforge-cli` | Clap CLI with daemon auto-detect (HTTP mode vs. in-process fallback), MCP server |
| `clawforge-migrate` | Migration engine for importing from OpenClaw (and future frameworks) |
| `clawforge-skills` | Skill system: 60 bundled skills, FangHub marketplace, OpenClaw compatibility, prompt injection scanning |
| `clawforge-desktop` | Tauri 2.0 native desktop app (WebView + system tray + single-instance + notifications) |
| `xtask` | Build automation tasks |

### Key Architectural Patterns

- **`KernelHandle` trait**: Defined in `clawforge-runtime`, implemented on `ClawForgeKernel` in `clawforge-kernel`. This avoids circular crate dependencies while enabling inter-agent tools.
- **Shared memory**: A fixed UUID (`AgentId(Uuid::from_bytes([0..0, 0x01]))`) provides a cross-agent KV namespace.
- **Daemon detection**: The CLI checks `~/.clawforge/daemon.json` and pings the health endpoint. If a daemon is running, commands use HTTP; otherwise, they boot an in-process kernel.
...
```bash
clawforge agent spawn agents/my-agent/agent.toml
```
...
Channel adapters live in `crates/clawforge-channels/src/`. Each adapter implements the `ChannelAdapter` trait.

### Steps

1. Create a new file: `crates/clawforge-channels/src/myplatform.rs`
...
3. Register the module in `crates/clawforge-channels/src/lib.rs`:

```rust
pub mod myplatform;
```

4. Wire it up in the channel bridge (`crates/clawforge-api/src/channel_bridge.rs`) so the daemon starts it alongside other adapters.

5. Add configuration support in `clawforge-types` config structs (add a `[channels.myplatform]` section).

6. Add CLI setup wizard instructions in `crates/clawforge-cli/src/main.rs` under `cmd_channel_setup`.
...
Built-in tools are defined in `crates/clawforge-runtime/src/tool_runner.rs`.
...
- Open a [GitHub Discussion](https://github.com/RightNow-AI/clawforge/discussions) for questions.
- Open a [GitHub Issue](https://github.com/RightNow-AI/clawforge/issues) for bugs or feature requests.
