...
// ---------------------------------------------------------------------------
// ClawForge output types (TOML)
// ---------------------------------------------------------------------------

/// ClawForge config.toml structure for serialization.
#[derive(Serialize)]
struct ClawForgeConfig {
    default_model: ClawForgeModelConfig,
    memory: ClawForgeMemorySection,
    network: ClawForgeNetworkSection,
    #[serde(skip_serializing_if = "Option::is_none")]
    channels: Option<toml::Value>,
}

#[derive(Serialize)]
struct ClawForgeModelConfig {
    provider: String,
    model: String,
    api_key_env: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Serialize)]
struct ClawForgeMemorySection {
    decay_rate: f32,
}

#[derive(Serialize)]
struct ClawForgeNetworkSection {
    listen_addr: String,
}
...
/// Map OpenClaw DM policy to ClawForge DM policy string.
...
/// Map OpenClaw group policy to ClawForge group policy string.
...
use clawforge_types::tool_compat::{is_known_clawforge_tool, map_tool_name};

/// Map OpenClaw tool profile to ClawForge capability tool list.
...
/// Map OpenClaw provider name to ClawForge provider name.
...
fn migrate_config_from_json(
    ...
    let of_config = ClawForgeConfig {
        ...
        memory: ClawForgeMemorySection { decay_rate: 0.05 },
        network: ClawForgeNetworkSection {
    ...
    let config_content = format!(
        "# ClawForge Agent OS configuration\n\
...
fn migrate_channels_from_json(
    ...
    // --- BlueBubbles (skip — no ClawForge adapter) ---
    if oc_channels.bluebubbles.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: "bluebubbles".to_string(),
            reason: "No ClawForge adapter available — consider using the iMessage channel instead"
                .to_string(),
        });
    }
    ...
    for key in oc_channels.other.keys() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: key.clone(),
            reason: format!("Unknown channel '{key}' — not mapped to any ClawForge adapter"),
        });
    }
            ...
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{id}': tool '{tool}' has no ClawForge equivalent and was skipped"
                    ));
                }
...
fn convert_agent_from_json(
    ...
    let system_prompt = entry
        ...
        .unwrap_or_else(|| {
            format!(
                "You are {display_name}, an AI agent running on the ClawForge Agent OS. You are helpful, concise, and accurate."
            )
        });
    ...
    toml_str.push_str("author = \"clawforge\"\n");
...
fn report_skipped_features(root: &OpenClawRoot, source: &Path, report: &mut MigrationReport) {
    ...
    if root.cron.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Config,
            name: "cron".to_string(),
            reason: "Cron job scheduling not yet supported — use ClawForge's ScheduleMode::Periodic instead".to_string(),
        });
    }
    ...
    if root.hooks.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason: "Webhook hooks not supported — use ClawForge's event system instead".to_string(),
        });
    }
        ...
        if let Some(ref entries) = skills.entries {
            if !entries.is_empty() {
                report.skipped.push(SkippedItem {
                    ...
                    reason: "Skills must be reinstalled via `clawforge skill install`".to_string(),
                });
            }
        }
    }
    ...
    if source.join("memory-search").join("index.db").exists() {
        report.skipped.push(SkippedItem {
            ...
            reason: "SQLite vector index not portable — ClawForge will rebuild embeddings"
                .to_string(),
        });
    }
    ...
    if root.session.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason: "Session scope config differs — ClawForge uses per-agent sessions by default"
                .to_string(),
        });
    }
    ...
    if root.memory.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason:
                "Memory backend config not migrated — ClawForge uses SQLite with vector embeddings"
                    .to_string(),
        });
    }
}
        ...
        match *name {
            ...
            "bluebubbles" => {
                report.skipped.push(SkippedItem {
                    kind: ItemKind::Channel,
                    name: "bluebubbles".to_string(),
                    reason: "No ClawForge adapter available — consider using the iMessage channel instead".to_string(),
                });
            }
            _ => {}
        }
    ...
    for entry in entries {
        ...
        match convert_legacy_agent(&agent_yaml, &agent_name) {
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{agent_name}': tool '{tool}' has no ClawForge equivalent and was skipped"
                    ));
                }
...
fn convert_legacy_agent(
    ...
    let system_prompt = oc.system_prompt.unwrap_or_else(|| {
        format!(
            "You are {}, an AI agent running on the ClawForge Agent OS. {}",
            oc.name,
            ...
            for entry in entries.flatten() {
                ...
                if has_package_json && has_index {
                    report.skipped.push(SkippedItem {
                        kind: ItemKind::Skill,
                        name: name.clone(),
                        reason: "Node.js skill — run with `clawforge skill install` after migration"
                            .to_string(),
                    });
                } else {
                    report.skipped.push(SkippedItem {
...
mod tests {
  ...
  agents: {
    ...
    #[test]
    fn test_is_known_clawforge_tool() {
        assert!(is_known_clawforge_tool("file_read"));
        assert!(is_known_clawforge_tool("shell_exec"));
        assert!(is_known_clawforge_tool("web_fetch"));
        assert!(!is_known_clawforge_tool("Read"));
        assert!(!is_known_clawforge_tool("unknown"));
    }
