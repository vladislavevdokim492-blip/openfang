...
use dashmap::DashMap;
use clawforge_types::agent::{AgentEntry, AgentId, AgentMode, AgentState};
...
impl AgentRegistry {
    ...
    pub fn register(&self, entry: AgentEntry) -> ClawForgeResult<()> {
        if self.name_index.contains_key(&entry.name) {
            return Err(ClawForgeError::AgentAlreadyExists(entry.name.clone()));
        }
        let id = entry.id;
    ...
    /// Update agent state.
    pub fn set_state(&self, id: AgentId, state: AgentState) -> ClawForgeResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| ClawForgeError::AgentNotFound(id.to_string()))?;
        entry.state = state;
    ...
    pub fn set_mode(&self, id: AgentId, mode: AgentMode) -> ClawForgeResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| ClawForgeError::AgentNotFound(id.to_string()))?;
    ...
    pub fn remove(&self, id: AgentId) -> ClawForgeResult<AgentEntry> {
        let (_, entry) = self
            .agents
            .remove(&id)
            .ok_or_else(|| ClawForgeError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_session_id(
        &self,
        id: AgentId,
        new_session_id: clawforge_types::agent::SessionId,
    ) -> ClawForgeResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_identity(
        &self,
        id: AgentId,
        identity: clawforge_types::agent::AgentIdentity,
    ) -> ClawForgeResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_model(&self, id: AgentId, new_model: String) -> ClawForgeResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| ClawForgeError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_fallback_models(
        &self,
        id: AgentId,
        fallback_models: Vec<clawforge_types::agent::FallbackModel>,
    ) -> ClawForgeResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_skills(&self, id: AgentId, skills: Vec<String>) -> ClawForgeResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| ClawForgeError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_mcp_servers(&self, id: AgentId, servers: Vec<String>) -> ClawForgeResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| ClawForgeError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_system_prompt(&self, id: AgentId, new_prompt: String) -> ClawForgeResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| ClawForgeError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_name(&self, id: AgentId, new_name: String) -> ClawForgeResult<()> {
        if self.name_index.contains_key(&new_name) {
            return Err(ClawForgeError::AgentAlreadyExists(new_name));
        }
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_description(&self, id: AgentId, new_desc: String) -> ClawForgeResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| ClawForgeError::AgentNotFound(id.to_string()))?;
    ...
    pub fn mark_onboarding_complete(&self, id: AgentId) -> ClawForgeResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| ClawForgeError::AgentNotFound(id.to_string()))?;
