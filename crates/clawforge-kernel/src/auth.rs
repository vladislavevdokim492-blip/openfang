...
//! to ClawForge users with roles, then enforces permission checks on actions.

use dashmap::DashMap;
use clawforge_types::agent::UserId;
use clawforge_types::config::UserConfig;
...
pub struct AuthManager {
    /// Known users by their ClawForge user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the ClawForge UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> ClawForgeResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| ClawForgeError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(ClawForgeError::AuthDenied(format!(
