//! Configuration loading from `~/.clawforge/config.toml` with defaults.
//!
...
///
/// Respects `CLAWFORGE_HOME` env var (e.g. `CLAWFORGE_HOME=/opt/clawforge`).
pub fn default_config_path() -> PathBuf {
    clawforge_home().join("config.toml")
}

/// Get the ClawForge home directory.
///
/// Priority: `CLAWFORGE_HOME` env var > `~/.clawforge`.
pub fn clawforge_home() -> PathBuf {
