...
use clawforge_memory::usage::{ModelUsage, UsageRecord, UsageStore, UsageSummary};
use clawforge_types::agent::{AgentId, ResourceQuota};
...
impl MeteringEngine {
    ...
    pub fn check_quota(&self, agent_id: AgentId, quota: &ResourceQuota) -> ClawForgeResult<()> {
        // Hourly check
        if quota.max_cost_per_hour_usd > 0.0 {
            ...
            if hourly_cost >= quota.max_cost_per_hour_usd {
                return Err(ClawForgeError::QuotaExceeded(format!(
    ...
    pub fn budget_status(&self, budget: &clawforge_types::config::BudgetConfig) -> BudgetStatus {
    ...
    pub fn get_summary(&self, agent_id: Option<AgentId>) -> ClawForgeResult<UsageSummary> {
        self.store.query_summary(agent_id)
    }

    /// Get usage grouped by model.
    pub fn get_by_model(&self) -> ClawForgeResult<Vec<ModelUsage>> {
    ...
    pub fn estimate_cost_with_catalog(
        catalog: &clawforge_runtime::model_catalog::ModelCatalog,
        model: &str,
        input_tokens: u64,
        output_tokens: u64,
    ) -> f64 {
    ...
    pub fn cleanup(&self, days: u32) -> ClawForgeResult<usize> {
