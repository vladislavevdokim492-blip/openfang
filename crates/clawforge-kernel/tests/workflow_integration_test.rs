...
async fn test_workflow_register_and_resolve() {
    ...
    let kernel = ClawForgeKernel::boot_with_config(config).expect("Kernel should boot");
...
async fn test_workflow_e2e_with_groq() {
    ...
    assert!(matches!(
        run.state,
        clawforge_kernel::workflow::WorkflowRunState::Completed
    ));
