...
//!
//! Run with: GROQ_API_KEY=gsk_... cargo test -p clawforge-kernel --test multi_agent_test -- --nocapture

use clawforge_kernel::ClawForgeKernel;
use clawforge_types::agent::AgentManifest;
use clawforge_types::config::{DefaultModelConfig, KernelConfig};

fn test_config() -> KernelConfig {
    let tmp = std::env::temp_dir().join("clawforge-multi-agent-test");
...
async fn test_six_agent_fleet() {
    ...
    let kernel = ClawForgeKernel::boot_with_config(test_config()).expect("Kernel should boot");
...
"#,
    ...
    println!("\n{}", "=".repeat(60));
    println!("  CLAWFORGE MULTI-AGENT FLEET TEST");
