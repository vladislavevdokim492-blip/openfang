...
pub fn default_client_ids() -> HashMap<&'static str, &'static str> {
    ...
    m.insert("google", "clawforge-google-client-id");
    m.insert("github", "clawforge-github-client-id");
    m.insert("microsoft", "clawforge-microsoft-client-id");
    m.insert("slack", "clawforge-slack-client-id");
    m
}

/// Resolve OAuth client IDs with config overrides applied on top of defaults.
pub fn resolve_client_ids(
    config: &clawforge_types::config::OAuthConfig,
...
mod tests {
    ...
    fn resolve_client_ids_uses_defaults() {
        let config = clawforge_types::config::OAuthConfig::default();
        let ids = resolve_client_ids(&config);
        assert_eq!(ids["google"], "clawforge-google-client-id");
        assert_eq!(ids["github"], "clawforge-github-client-id");
    }

    #[test]
    fn resolve_client_ids_applies_overrides() {
        let config = clawforge_types::config::OAuthConfig {
        ...
        assert_eq!(ids["github"], "clawforge-github-client-id"); // default
        assert_eq!(ids["microsoft"], "my-msft-id");
        assert_eq!(ids["slack"], "clawforge-slack-client-id"); // default
    }
}
