// ClawForge App — Alpine.js init, hash router, global store
'use strict';
...
document.addEventListener('alpine:init', function() {
  ...
  var savedKey = localStorage.getItem('clawforge-api-key');
  if (savedKey) ClawForgeAPI.setAuthToken(savedKey);
  ...
  Alpine.store('app', {
    ...
    agentCount: 0,
    pendingAgent: null,
    focusMode: localStorage.getItem('clawforge-focus') === 'true',
    showOnboarding: false,
    showAuthPrompt: false,

    toggleFocusMode() {
      this.focusMode = !this.focusMode;
      localStorage.setItem('clawforge-focus', this.focusMode);
    },

    async refreshAgents() {
      try {
        var agents = await ClawForgeAPI.get('/api/agents');
    ...
    async checkStatus() {
      try {
        var s = await ClawForgeAPI.get('/api/status');
        this.connected = true;
        this.booting = false;
        this.lastError = '';
        ...
        console.warn('[ClawForge] Status check failed:', e.message);
      }
    },

    async checkOnboarding() {
      if (localStorage.getItem('clawforge-onboarded')) return;
      try {
        var config = await ClawForgeAPI.get('/api/config');
    ...
    async checkAuth() {
      try {
        ...
        await ClawForgeAPI.get('/api/tools');
        ...
        if (e.message && (e.message.indexOf('Not authorized') >= 0 || e.message.indexOf('401') >= 0 || e.message.indexOf('Missing Authorization') >= 0 || e.message.indexOf('Unauthorized') >= 0)) {
          ...
          var saved = localStorage.getItem('clawforge-api-key');
          if (saved) {
            // Saved key might be stale — clear it and show prompt
            ClawForgeAPI.setAuthToken('');
            localStorage.removeItem('clawforge-api-key');
          }
    ...
    submitApiKey(key) {
      ...
      ClawForgeAPI.setAuthToken(key.trim());
      localStorage.setItem('clawforge-api-key', key.trim());
      this.showAuthPrompt = false;
      this.refreshAgents();
    },
...
function app() {
  return {
    page: 'agents',
    themeMode: localStorage.getItem('clawforge-theme-mode') || 'system',
    theme: (() => {
      var mode = localStorage.getItem('clawforge-theme-mode') || 'system';
      if (mode === 'system') return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
      return mode;
    })(),
    sidebarCollapsed: localStorage.getItem('clawforge-sidebar') === 'collapsed',
    ...
    init() {
      ...
      ClawForgeAPI.onConnectionChange(function(state) {
    ...
    setTheme(mode) {
      this.themeMode = mode;
      localStorage.setItem('clawforge-theme-mode', mode);
      if (mode === 'system') {
    ...
    toggleSidebar() {
      ...
      localStorage.setItem('clawforge-sidebar', this.sidebarCollapsed ? 'collapsed' : 'expanded');
    },
    ...
    async pollStatus() {
      ...
      this.wsConnected = ClawForgeAPI.isWsConnected();
    }
  };
}
