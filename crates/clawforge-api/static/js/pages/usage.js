// ClawForge Analytics Page — Full usage analytics with per-model and per-agent breakdowns
...
function analyticsPage() {
  return {
    ...
    async loadSummary() {
      try {
        this.summary = await ClawForgeAPI.get('/api/usage/summary');
      } catch(e) {
    ...
    async loadByModel() {
      try {
        var data = await ClawForgeAPI.get('/api/usage/by-model');
    ...
    async loadDailyCosts() {
      try {
        var data = await ClawForgeAPI.get('/api/usage/daily');
