// ClawForge Settings Page — Provider Hub, Model Catalog, Config, Tools + Security, Network, Migration tabs
'use strict';
  ...
  return {
    ...
    configurableFeatures: [
      ...
      },
      {
        ...
        configHint: 'Set api_key in ~/.clawforge/config.toml for remote access. Empty = localhost only.',
        valueKey: 'auth'
      }
    ],
    ...
    async loadSysInfo() {
      try {
        var ver = await ClawForgeAPI.get('/api/version');
        var status = await ClawForgeAPI.get('/api/status');
        this.sysInfo = {
          version: ver.version || '-',
    ...
    async loadUsage() {
      try {
        var data = await ClawForgeAPI.get('/api/usage');
    ...
    async loadTools() {
      try {
        var data = await ClawForgeAPI.get('/api/tools');
    ...
    async loadConfig() {
      try {
        this.config = await ClawForgeAPI.get('/api/config');
      } catch(e) { this.config = {}; }
    },

    async loadProviders() {
      try {
        var data = await ClawForgeAPI.get('/api/providers');
    ...
    async loadModels() {
      try {
        var data = await ClawForgeAPI.get('/api/models');
    ...
    async addCustomModel() {
      ...
      try {
        await ClawForgeAPI.post('/api/models/custom', {
          id: id,
    ...
    async deleteCustomModel(modelId) {
      ...
      try {
        await ClawForgeAPI.del('/api/models/custom/' + encodeURIComponent(modelId));
        ClawForgeToast.success('Model deleted');
        await this.loadModels();
      } catch(e) {
        ClawForgeToast.error('Failed to delete: ' + (e.message || 'Unknown error'));
      }
    },
    ...
    async loadConfigSchema() {
      try {
        var results = await Promise.all([
          ClawForgeAPI.get('/api/config/schema').catch(function() { return {}; }),
          ClawForgeAPI.get('/api/config')
        ]);
    ...
    async saveConfigField(section, field, value) {
      ...
      try {
        await ClawForgeAPI.post('/api/config/set', { path: key, value: value });
        this.configDirty[key] = false;
        ClawForgeToast.success('Saved ' + key);
      } catch(e) {
        ClawForgeToast.error('Failed to save: ' + e.message);
      }
    ...
    async saveProviderKey(provider) {
      ...
      if (!key || !key.trim()) { ClawForgeToast.error('Please enter an API key'); return; }
      try {
        await ClawForgeAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key.trim() });
        ClawForgeToast.success('API key saved for ' + provider.display_name);
        this.providerKeyInputs[provider.id] = '';
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        ClawForgeToast.error('Failed to save key: ' + e.message);
      }
    },

    async removeProviderKey(provider) {
      try {
        await ClawForgeAPI.del('/api/providers/' + encodeURIComponent(provider.id) + '/key');
        ClawForgeToast.success('API key removed for ' + provider.display_name);
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        ClawForgeToast.error('Failed to remove key: ' + e.message);
      }
    },
    ...
    async startCopilotOAuth() {
      ...
      try {
        var resp = await ClawForgeAPI.post('/api/providers/github-copilot/oauth/start', {});
        ...
        ClawForgeToast.error('Failed to start Copilot login: ' + e.message);
    ...
    pollCopilotOAuth() {
      ...
      setTimeout(async function() {
        ...
        try {
          var resp = await ClawForgeAPI.get('/api/providers/github-copilot/oauth/poll/' + self.copilotOAuth.pollId);
          if (resp.status === 'complete') {
            ClawForgeToast.success('GitHub Copilot authenticated successfully!');
            ...
            ClawForgeToast.error('Device code expired. Please try again.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else if (resp.status === 'denied') {
            ClawForgeToast.error('Access denied by user.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else {
            ClawForgeToast.error('OAuth error: ' + (resp.error || resp.status));
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          }
        } catch(e) {
          ClawForgeToast.error('Poll error: ' + e.message);
          self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
        }
    ...
    async testProvider(provider) {
      ...
      try {
        var result = await ClawForgeAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/test', {});
        this.providerTestResults[provider.id] = result;
        if (result.status === 'ok') {
          ClawForgeToast.success(provider.display_name + ' connected (' + (result.latency_ms || '?') + 'ms)');
        } else {
          ClawForgeToast.error(provider.display_name + ': ' + (result.error || 'Connection failed'));
        }
      } catch(e) {
        this.providerTestResults[provider.id] = { status: 'error', error: e.message };
        ClawForgeToast.error('Test failed: ' + e.message);
      }
    ...
    async saveProviderUrl(provider) {
      ...
      if (!url || !url.trim()) { ClawForgeToast.error('Please enter a base URL'); return; }
      url = url.trim();
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        ClawForgeToast.error('URL must start with http:// or https://'); return;
      }
      this.providerUrlSaving[provider.id] = true;
      try {
        var result = await ClawForgeAPI.put('/api/providers/' + encodeURIComponent(provider.id) + '/url', { base_url: url });
        if (result.reachable) {
          ClawForgeToast.success(provider.display_name + ' URL saved &mdash; reachable (' + (result.latency_ms || '?') + 'ms)');
        } else {
          ClawForgeToast.warning(provider.display_name + ' URL saved but not reachable');
        }
        await this.loadProviders();
      } catch(e) {
        ClawForgeToast.error('Failed to save URL: ' + e.message);
      }
    ...
    async addCustomProvider() {
      ...
      if (!name) { ClawForgeToast.error('Please enter a provider name'); return; }
      var url = this.customProviderUrl.trim();
      if (!url) { ClawForgeToast.error('Please enter a base URL'); return; }
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        ClawForgeToast.error('URL must start with http:// or https://'); return;
      }
      ...
      try {
        var result = await ClawForgeAPI.put('/api/providers/' + encodeURIComponent(name) + '/url', { base_url: url });
        if (this.customProviderKey.trim()) {
          await ClawForgeAPI.post('/api/providers/' + encodeURIComponent(name) + '/key', { key: this.customProviderKey.trim() });
        }
        this.customProviderName = '';
        this.customProviderUrl = '';
        this.customProviderKey = '';
        this.customProviderStatus = '';
        ClawForgeToast.success('Provider "' + name + '" added' + (result.reachable ? ' (reachable)' : ' (not reachable yet)'));
        await this.loadProviders();
      } catch(e) {
        this.customProviderStatus = 'Error: ' + (e.message || 'Failed');
        ClawForgeToast.error('Failed to add provider: ' + e.message);
      }
    ...
    async loadSecurity() {
      ...
      try {
        this.securityData = await ClawForgeAPI.get('/api/security');
      } catch(e) {
    ...
    async verifyAuditChain() {
      ...
      try {
        var res = await ClawForgeAPI.get('/api/audit/verify');
    ...
    async loadPeers() {
      ...
      this.peersLoadError = '';
      try {
        var data = await ClawForgeAPI.get('/api/peers');
    ...
    async autoDetect() {
      this.detecting = true;
      try {
        var data = await ClawForgeAPI.get('/api/migrate/detect');
    ...
    async scanPath() {
      ...
      this.scanning = true;
      try {
        var data = await ClawForgeAPI.post('/api/migrate/scan', { path: this.sourcePath });
        if (data.error) {
          ClawForgeToast.error('Scan error: ' + data.error);
          this.scanning = false;
          return;
        }
        ...
        ClawForgeToast.error('Scan failed: ' + e.message);
      }
      this.scanning = false;
    },
    ...
    async runMigration(dryRun) {
      ...
      try {
        ...
        if (!target) target = '';
        var data = await ClawForgeAPI.post('/api/migrate', {
          source: 'openclaw',
