// ClawForge Comms Page — Agent topology & inter-agent communication feed
'use strict';
...
function commsPage() {
  return {
    ...
    async loadData() {
      ...
      try {
        var results = await Promise.all([
          ClawForgeAPI.get('/api/comms/topology'),
          ClawForgeAPI.get('/api/comms/events?limit=200')
        ]);
    ...
    startSSE() {
      ...
      var self = this;
      var url = ClawForgeAPI.baseUrl + '/api/comms/events/stream';
      if (ClawForgeAPI.apiKey) url += '?token=' + encodeURIComponent(ClawForgeAPI.apiKey);
    ...
    async refreshTopology() {
      try {
        this.topology = await ClawForgeAPI.get('/api/comms/topology');
      } catch(e) { /* silent */ }
    },
    ...
    async submitSend() {
      ...
      try {
        await ClawForgeAPI.post('/api/comms/send', {
          from_agent_id: this.sendFrom,
          to_agent_id: this.sendTo,
          message: this.sendMsg
        });
        ClawForgeToast.success('Message sent');
        this.showSendModal = false;
      } catch(e) {
        ClawForgeToast.error(e.message || 'Send failed');
      }
    ...
    async submitTask() {
      ...
      try {
        ...
        await ClawForgeAPI.post('/api/comms/task', body);
        ClawForgeToast.success('Task posted');
        this.showTaskModal = false;
      } catch(e) {
        ClawForgeToast.error(e.message || 'Task failed');
      }
