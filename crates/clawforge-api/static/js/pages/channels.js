// ClawForge Channels Page — OpenClaw-style setup UX with QR code support
'use strict';
...
function channelsPage() {
  return {
    ...
    async loadChannels() {
      ...
      this.loadError = '';
      try {
        var data = await ClawForgeAPI.get('/api/channels');
    ...
    async startQR() {
      ...
      try {
        var result = await ClawForgeAPI.post('/api/channels/whatsapp/qr/start', {});
        ...
        if (this.qr.connected) {
          ClawForgeToast.success('WhatsApp connected!');
    ...
    pollQR() {
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          var result = await ClawForgeAPI.get('/api/channels/whatsapp/qr/status?session_id=' + encodeURIComponent(self.qr.sessionId));
          if (result.connected) {
            clearInterval(self.qrPollTimer);
            self.qrPollTimer = null;
            self.qr.connected = true;
            self.qr.message = result.message || 'Connected!';
            ClawForgeToast.success('WhatsApp linked successfully!');
    ...
    async saveChannel() {
      ...
      try {
        await ClawForgeAPI.post('/api/channels/' + name + '/configure', {
          fields: this.formValues
        });
        this.setupStep = 2;
        // Auto-test after save
        try {
          var testResult = await ClawForgeAPI.post('/api/channels/' + name + '/test', {});
          if (testResult.status === 'ok') {
            this.testPassed = true;
            this.setupStep = 3;
            ClawForgeToast.success(this.setupModal.display_name + ' activated!');
          } else {
            ClawForgeToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          ClawForgeToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
    ...
    async removeChannel() {
      ...
      var self = this;
      ClawForgeToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await ClawForgeAPI.delete('/api/channels/' + name + '/configure');
          ClawForgeToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        var result = await ClawForgeAPI.post('/api/channels/' + name + '/test', {});
        if (result.status === 'ok') {
          this.testPassed = true;
          this.setupStep = 3;
          ClawForgeToast.success(result.message);
        } else {
          ClawForgeToast.error(result.message);
        }
      } catch(e) {
        ClawForgeToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        ClawForgeToast.success('Copied to clipboard');
      } catch(e) {
        ClawForgeToast.error('Copy failed');
      }
    },
