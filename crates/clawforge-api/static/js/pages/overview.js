// ClawForge Overview Dashboard — Landing page with system stats + provider status
'use strict';
...
function overviewPage() {
  return {
    ...
    async loadHealth() {
      try {
        this.health = await ClawForgeAPI.get('/api/health');
      } catch(e) { this.health = { status: 'unreachable' }; }
    },

    async loadStatus() {
      try {
        this.status = await ClawForgeAPI.get('/api/status');
    ...
    async loadAudit() {
      try {
        var data = await ClawForgeAPI.get('/api/audit/recent?n=8');
