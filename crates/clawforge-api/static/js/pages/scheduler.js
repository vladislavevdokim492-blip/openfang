// ClawForge Scheduler Page — Cron job management + event triggers unified view
'use strict';
...
function schedulerPage() {
  return {
    ...
    async loadJobs() {
      var data = await ClawForgeAPI.get('/api/cron/jobs');
      var raw = data.jobs || [];
    ...
    async loadTriggers() {
      ...
      this.trigLoadError = '';
      try {
        var data = await ClawForgeAPI.get('/api/triggers');
    ...
    async createJob() {
      if (!this.newJob.name.trim()) {
        ClawForgeToast.warn('Please enter a job name');
        return;
      }
      if (!this.newJob.cron.trim()) {
        ClawForgeToast.warn('Please enter a cron expression');
        return;
      }
      this.creating = true;
      try {
        ...
        await ClawForgeAPI.post('/api/cron/jobs', body);
        this.showCreateForm = false;
        this.newJob = { name: '', cron: '', agent_id: '', message: '', enabled: true };
        ClawForgeToast.success('Schedule "' + jobName + '" created');
        await this.loadJobs();
      } catch(e) {
        ClawForgeToast.error('Failed to create schedule: ' + (e.message || e));
      }
      this.creating = false;
    },
    ...
    async toggleJob(job) {
      try {
        ...
        await ClawForgeAPI.put('/api/cron/jobs/' + job.id + '/enable', { enabled: newState });
        job.enabled = newState;
        ClawForgeToast.success('Schedule ' + (newState ? 'enabled' : 'paused'));
      } catch(e) {
        ClawForgeToast.error('Failed to toggle schedule: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      ClawForgeToast.confirm('Delete Schedule', 'Delete "' + jobName + '"? This cannot be undone.', async function() {
        try {
          await ClawForgeAPI.del('/api/cron/jobs/' + job.id);
          self.jobs = self.jobs.filter(function(j) { return j.id !== job.id; });
          ClawForgeToast.success('Schedule "' + jobName + '" deleted');
        } catch(e) {
          ClawForgeToast.error('Failed to delete schedule: ' + (e.message || e));
        }
      });
    },
    ...
    async runNow(job) {
      ...
      try {
        var result = await ClawForgeAPI.post('/api/schedules/' + job.id + '/run', {});
        if (result.status === 'completed') {
          ClawForgeToast.success('Schedule "' + (job.name || 'job') + '" executed successfully');
          job.last_run = new Date().toISOString();
        } else {
          ClawForgeToast.error('Schedule run failed: ' + (result.error || 'Unknown error'));
        }
      } catch(e) {
        ClawForgeToast.error('Run Now is not yet available for cron jobs');
      }
      this.runningJobId = '';
    },
    ...
    async toggleTrigger(trigger) {
      try {
        ...
        await ClawForgeAPI.put('/api/triggers/' + trigger.id, { enabled: newState });
        trigger.enabled = newState;
        ClawForgeToast.success('Trigger ' + (newState ? 'enabled' : 'disabled'));
      } catch(e) {
        ClawForgeToast.error('Failed to toggle trigger: ' + (e.message || e));
      }
    },

    deleteTrigger(trigger) {
      var self = this;
      ClawForgeToast.confirm('Delete Trigger', 'Delete this trigger? This cannot be undone.', async function() {
        try {
          await ClawForgeAPI.del('/api/triggers/' + trigger.id);
          self.triggers = self.triggers.filter(function(t) { return t.id !== trigger.id; });
          ClawForgeToast.success('Trigger deleted');
        } catch(e) {
          ClawForgeToast.error('Failed to delete trigger: ' + (e.message || e));
        }
      });
    },
