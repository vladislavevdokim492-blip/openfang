// ClawForge Setup Wizard — First-run guided setup (Provider + Agent + Channel)
'use strict';
...
function wizardPage() {
  return {
    ...
    async sendTryItMessage(text) {
      ...
      try {
        var res = await ClawForgeAPI.post('/api/agents/' + this.createdAgent.id + '/message', { message: text });
    ...
    async saveKey() {
      ...
      if (!key) {
        ClawForgeToast.error('Please enter an API key');
        return;
      }
      this.savingKey = true;
      try {
        await ClawForgeAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key });
        this.apiKeyInput = '';
        this.keySaved = true;
    ...
    async createAgent() {
      ...
      if (!name) {
        ClawForgeToast.error('Please enter a name for your agent');
        return;
      }
      ...
      try {
        ...
        if (res.agent_id) {
          ...
          ClawForgeToast.success('Agent "' + (res.name || name) + '" created');
          await Alpine.store('app').refreshAgents();
        } else {
          ClawForgeToast.error('Failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        ClawForgeToast.error('Failed to create agent: ' + e.message);
      }
    ...
    async configureChannel() {
      ...
      if (!token) {
        ClawForgeToast.error('Please enter the ' + ch.token_label);
        return;
      }
      ...
      try {
        ...
        fields.token = token;
        await ClawForgeAPI.post('/api/channels/' + ch.name + '/configure', { fields: fields });
        this.channelConfigured = true;
        this.setupSummary.channel = ch.display_name;
        ClawForgeToast.success(ch.display_name + ' configured and activated.');
      } catch(e) {
        ClawForgeToast.error('Failed: ' + (e.message || 'Unknown error'));
      }
    ...
    finish() {
      localStorage.setItem('clawforge-onboarded', 'true');
