// ClawForge Chat Page — Agent chat with markdown + streaming
'use strict';
  ...
  return {
      ...
      this.$watch('inputText', function(val) {
        ...
        if (modelMatch) {
          ...
          if (!self.modelPickerList.length) {
            ClawForgeAPI.get('/api/models').then(function(data) {
    ...
    toggleModelSwitcher() {
      ...
      ClawForgeAPI.get('/api/models').then(function(data) {
        ...
        ClawForgeToast.error('Failed to load models: ' + e.message);
      });
    },
    ...
    switchModel(model) {
      ...
      ClawForgeAPI.put('/api/agents/' + this.currentAgent.id + '/model', { model: model.id }).then(function() {
        self.currentAgent.model_name = model.id;
        self.currentAgent.model_provider = model.provider;
        ClawForgeToast.success('Switched to ' + (model.display_name || model.id));
        self.showModelSwitcher = false;
        self.modelSwitching = false;
      }).catch(function(e) {
        ClawForgeToast.error('Switch failed: ' + e.message);
        self.modelSwitching = false;
      });
    },
    ...
    fetchCommands: function() {
      var self = this;
      ClawForgeAPI.get('/api/commands').then(function(data) {
      ...
      switch (cmd) {
        ...
        case '/new':
          if (self.currentAgent) {
            ClawForgeAPI.post('/api/agents/' + self.currentAgent.id + '/session/reset', {}).then(function() {
              self.messages = [];
              ClawForgeToast.success('Session reset');
            }).catch(function(e) { ClawForgeToast.error('Reset failed: ' + e.message); });
          }
          break;
        case '/compact':
          if (self.currentAgent) {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Compacting session...', meta: '', tools: [] });
            ClawForgeAPI.post('/api/agents/' + self.currentAgent.id + '/session/compact', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Compaction complete', meta: '', tools: [] });
              self.scrollToBottom();
            }).catch(function(e) { ClawForgeToast.error('Compaction failed: ' + e.message); });
          }
          break;
        case '/stop':
          if (self.currentAgent) {
            ClawForgeAPI.post('/api/agents/' + self.currentAgent.id + '/stop', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Run cancelled', meta: '', tools: [] });
              self.sending = false;
              self.scrollToBottom();
            }).catch(function(e) { ClawForgeToast.error('Stop failed: ' + e.message); });
          }
          break;
        case '/usage':
          if (self.currentAgent) {
        ...
        case '/context':
          // Send via WS command
          if (self.currentAgent && ClawForgeAPI.isWsConnected()) {
            ClawForgeAPI.wsSend({ type: 'command', command: 'context', args: '' });
          } else {
        ...
        case '/verbose':
          if (self.currentAgent && ClawForgeAPI.isWsConnected()) {
            ClawForgeAPI.wsSend({ type: 'command', command: 'verbose', args: cmdArgs });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected. Connect to an agent first.', meta: '', tools: [] });
            self.scrollToBottom();
          }
        ...
        case '/queue':
          if (self.currentAgent && ClawForgeAPI.isWsConnected()) {
            ClawForgeAPI.wsSend({ type: 'command', command: 'queue', args: '' });
          } else {
          ...
          break;
        case '/status':
          ClawForgeAPI.get('/api/status').then(function(s) {
        ...
        case '/model':
          if (self.currentAgent) {
            if (cmdArgs) {
              ClawForgeAPI.put('/api/agents/' + self.currentAgent.id + '/model', { model: cmdArgs }).then(function(resp) {
                ...
                self.scrollToBottom();
              }).catch(function(e) { ClawForgeToast.error('Model switch failed: ' + e.message); });
            } else {
        ...
        case '/exit':
          ...
          break;
        case '/budget':
          ClawForgeAPI.get('/api/budget').then(function(b) {
          ...
          break;
        case '/peers':
          ClawForgeAPI.get('/api/network/status').then(function(ns) {
          ...
          break;
        case '/a2a':
          ClawForgeAPI.get('/api/a2a/agents').then(function(res) {
    ...
    selectAgent(agent) {
      ...
      if (!localStorage.getItem('of-chat-tips-seen')) {
        ...
        this.messages.push({
          id: ++localMsgId,
          role: 'system',
          text: '**Welcome to ClawForge Chat!**\n\n' +
    ...
    async loadSession(agentId) {
      var self = this;
      try {
        var data = await ClawForgeAPI.get('/api/agents/' + agentId + '/session');
    ...
    async loadSessions(agentId) {
      try {
        var data = await ClawForgeAPI.get('/api/agents/' + agentId + '/sessions');
    ...
    async createSession() {
      ...
      try {
        await ClawForgeAPI.post('/api/agents/' + this.currentAgent.id + '/sessions', {
        ...
        this.messages = [];
        this.scrollToBottom();
        if (typeof ClawForgeToast !== 'undefined') ClawForgeToast.success('New session created');
      } catch(e) {
        if (typeof ClawForgeToast !== 'undefined') ClawForgeToast.error('Failed to create session');
      }
    },
    ...
    async switchSession(sessionId) {
      if (!this.currentAgent) return;
      try {
        await ClawForgeAPI.post('/api/agents/' + this.currentAgent.id + '/sessions/' + sessionId + '/switch', {});
        this.messages = [];
        await this.loadSession(this.currentAgent.id);
        await this.loadSessions(this.currentAgent.id);
        ...
        if (typeof ClawForgeToast !== 'undefined') ClawForgeToast.error('Failed to switch session');
      }
    },
    ...
    connectWs(agentId) {
      ...
      ClawForgeAPI.wsConnect(agentId, {
        onOpen: function() {
    ...
    async sendMessage() {
      ...
      if (this.attachments.length) {
        for (var i = 0; i < this.attachments.length; i++) {
          ...
          att.uploading = true;
          try {
            var uploadRes = await ClawForgeAPI.upload(this.currentAgent.id, att.file);
            ...
            ClawForgeToast.error('Failed to upload ' + att.file.name);
    ...
    async _sendPayload(finalText, uploadedFiles, msgImages) {
      ...
      if (ClawForgeAPI.wsSend(wsPayload)) {
      ...
      // HTTP fallback
      if (!ClawForgeAPI.isWsConnected()) {
        ClawForgeToast.info('Using HTTP mode (no streaming)');
      }
      ...
      try {
        ...
        var res = await ClawForgeAPI.post('/api/agents/' + this.currentAgent.id + '/message', httpBody);
    ...
    stopAgent: function() {
      if (!this.currentAgent) return;
      var self = this;
      ClawForgeAPI.post('/api/agents/' + this.currentAgent.id + '/stop', {}).then(function(res) {
    ...
    killAgent() {
      ...
      ClawForgeToast.confirm('Stop Agent', 'Stop agent "' + name + '"? The agent will be shut down.', async function() {
        try {
          await ClawForgeAPI.del('/api/agents/' + self.currentAgent.id);
          ClawForgeAPI.wsDisconnect();
          self._wsAgent = null;
          self.currentAgent = null;
          self.messages = [];
          ClawForgeToast.success('Agent "' + name + '" stopped');
    ...
    addFiles(files) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (file.size > 10 * 1024 * 1024) {
          ClawForgeToast.warn('File "' + file.name + '" exceeds 10MB limit');
          continue;
        }
        ...
        if (!typeOk) {
          ClawForgeToast.warn('File type not supported: ' + file.name);
          continue;
        }
        var preview = null;
    ...
    startRecording: async function() {
      ...
      try {
        ...
        if (typeof ClawForgeToast !== 'undefined') ClawForgeToast.error('Microphone access denied');
      }
    },
    ...
    _handleRecordingComplete: async function() {
      ...
      try {
        ...
        var upload = await ClawForgeAPI.upload(this.currentAgent.id, file);
        ...
        this.messages = this.messages.filter(function(m) { return !m.thinking || m.role !== 'system'; });
        if (typeof ClawForgeToast !== 'undefined') ClawForgeToast.error('Failed to upload audio: ' + (e.message || 'unknown error'));
      }
    },
