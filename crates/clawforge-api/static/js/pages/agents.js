// ClawForge Agents Page — Multi-step spawn wizard, detail view with tabs, file editor, personality presets
'use strict';
...
function agentsPage() {
  return {
    ...
    async loadSpawnProfiles() {
      ...
      try {
        var data = await ClawForgeAPI.get('/api/profiles');
    ...
    async loadTemplates() {
      ...
      try {
        var results = await Promise.all([
          ClawForgeAPI.get('/api/templates'),
          ClawForgeAPI.get('/api/providers').catch(function() { return { providers: [] }; })
        ]);
    ...
    async showDetail(agent) {
      ...
      try {
        var full = await ClawForgeAPI.get('/api/agents/' + agent.id);
    ...
    killAgent(agent) {
      var self = this;
      ClawForgeToast.confirm('Stop Agent', 'Stop agent "' + agent.name + '"? The agent will be shut down.', async function() {
        try {
          await ClawForgeAPI.del('/api/agents/' + agent.id);
          ClawForgeToast.success('Agent "' + agent.name + '" stopped');
          self.showDetailModal = false;
          await Alpine.store('app').refreshAgents();
        } catch(e) {
          ClawForgeToast.error('Failed to stop agent: ' + e.message);
        }
      });
    },
    ...
    killAllAgents() {
      ...
      ClawForgeToast.confirm('Stop All Agents', 'Stop ' + list.length + ' agent(s)? All agents will be shut down.', async function() {
        var errors = [];
        for (var i = 0; i < list.length; i++) {
          try {
            await ClawForgeAPI.del('/api/agents/' + list[i].id);
          } catch(e) { errors.push(list[i].name + ': ' + e.message); }
        }
        await Alpine.store('app').refreshAgents();
        if (errors.length) {
          ClawForgeToast.error('Some agents failed to stop: ' + errors.join(', '));
        } else {
          ClawForgeToast.success(list.length + ' agent(s) stopped');
        }
      });
    },
    ...
    nextStep() {
      if (this.spawnStep === 1 && !this.spawnForm.name.trim()) {
        ClawForgeToast.warn('Please enter an agent name');
        return;
      }
    ...
    async setMode(agent, mode) {
      try {
        await ClawForgeAPI.put('/api/agents/' + agent.id + '/mode', { mode: mode });
        agent.mode = mode;
        ClawForgeToast.success('Mode set to ' + mode);
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        ClawForgeToast.error('Failed to set mode: ' + e.message);
      }
    },
    ...
    async spawnAgent() {
      ...
      if (!toml.trim()) {
        this.spawning = false;
        ClawForgeToast.warn('Manifest is empty \u2014 enter agent config first');
        return;
      }

      try {
        var res = await ClawForgeAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          ...
          if (Object.keys(patchBody).length) {
            ClawForgeAPI.patch('/api/agents/' + res.agent_id + '/config', patchBody).catch(function(e) { console.warn('Post-spawn config patch failed:', e.message); });
          }
          if (this.soulContent.trim()) {
            ClawForgeAPI.put('/api/agents/' + res.agent_id + '/files/SOUL.md', { content: '# Soul\n' + this.soulContent }).catch(function(e) { console.warn('SOUL.md write failed:', e.message); });
          }
          ...
          this.spawnToml = '';
          this.spawnStep = 1;
          ClawForgeToast.success('Agent "' + (res.name || 'new') + '" spawned');
          await Alpine.store('app').refreshAgents();
          this.chatWithAgent({ id: res.agent_id, name: res.name, model_provider: '?', model_name: '?' });
        } else {
          ClawForgeToast.error('Spawn failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        ClawForgeToast.error('Failed to spawn agent: ' + e.message);
      }
      this.spawning = false;
    },
    ...
    async loadAgentFiles() {
      ...
      try {
        var data = await ClawForgeAPI.get('/api/agents/' + this.detailAgent.id + '/files');
        this.agentFiles = data.files || [];
      } catch(e) {
        this.agentFiles = [];
        ClawForgeToast.error('Failed to load files: ' + e.message);
      }
    ...
    async openFile(file) {
      ...
      try {
        var data = await ClawForgeAPI.get('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(file.name));
        this.editingFile = file.name;
        this.fileContent = data.content || '';
      } catch(e) {
        ClawForgeToast.error('Failed to read file: ' + e.message);
      }
    },
    ...
    async saveFile() {
      ...
      try {
        await ClawForgeAPI.put('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(this.editingFile), { content: this.fileContent });
        ClawForgeToast.success(this.editingFile + ' saved');
        await this.loadAgentFiles();
      } catch(e) {
        ClawForgeToast.error('Failed to save file: ' + e.message);
      }
    ...
    async saveConfig() {
      ...
      try {
        await ClawForgeAPI.patch('/api/agents/' + this.detailAgent.id + '/config', this.configForm);
        ClawForgeToast.success('Config updated');
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        ClawForgeToast.error('Failed to save config: ' + e.message);
      }
    ...
    async cloneAgent(agent) {
      ...
      try {
        var res = await ClawForgeAPI.post('/api/agents/' + agent.id + '/clone', { new_name: newName });
        if (res.agent_id) {
          ClawForgeToast.success('Cloned as "' + res.name + '"');
          await Alpine.store('app').refreshAgents();
          this.showDetailModal = false;
        }
      } catch(e) {
        ClawForgeToast.error('Clone failed: ' + e.message);
      }
    },
    ...
    async spawnFromTemplate(name) {
      try {
        var data = await ClawForgeAPI.get('/api/templates/' + encodeURIComponent(name));
        if (data.manifest_toml) {
          var res = await ClawForgeAPI.post('/api/agents', { manifest_toml: data.manifest_toml });
          if (res.agent_id) {
            ClawForgeToast.success('Agent "' + (res.name || name) + '" spawned from template');
            await Alpine.store('app').refreshAgents();
            this.chatWithAgent({ id: res.agent_id, name: res.name || name, model_provider: '?', model_name: '?' });
          }
        }
      } catch(e) {
        ClawForgeToast.error('Failed to spawn from template: ' + e.message);
      }
    },
    ...
    async clearHistory(agent) {
      var self = this;
      ClawForgeToast.confirm('Clear History', 'Clear all conversation history for "' + agent.name + '"? This cannot be undone.', async function() {
        try {
          await ClawForgeAPI.del('/api/agents/' + agent.id + '/history');
          ClawForgeToast.success('History cleared for "' + agent.name + '"');
        } catch(e) {
          ClawForgeToast.error('Failed to clear history: ' + e.message);
        }
      });
    },
    ...
    async changeModel() {
      ...
      try {
        await ClawForgeAPI.put('/api/agents/' + this.detailAgent.id + '/model', { model: this.newModelValue.trim() });
        ClawForgeToast.success('Model changed (memory reset)');
        ...
        ClawForgeToast.error('Failed to change model: ' + e.message);
      }
    ...
    async addFallback() {
      ...
      try {
        await ClawForgeAPI.patch('/api/agents/' + this.detailAgent.id + '/config', {
          fallback_models: this.detailAgent._fallbacks
        });
        ClawForgeToast.success('Fallback added: ' + provider + '/' + model);
      } catch(e) {
        ClawForgeToast.error('Failed to save fallbacks: ' + e.message);
    ...
    async removeFallback(idx) {
      ...
      try {
        ...
        ClawForgeToast.success('Fallback removed');
      } catch(e) {
        ClawForgeToast.error('Failed to save fallbacks: ' + e.message);
    ...
    async loadToolFilters() {
      ...
      try {
        this.toolFilters = await ClawForgeAPI.get('/api/agents/' + this.detailAgent.id + '/tools');
      } catch(e) {
    ...
    async saveToolFilters() {
      if (!this.detailAgent) return;
      try {
        await ClawForgeAPI.put('/api/agents/' + this.detailAgent.id + '/tools', this.toolFilters);
      } catch(e) {
        ClawForgeToast.error('Failed to update tool filters: ' + e.message);
      }
    },
    ...
    async spawnBuiltin(t) {
      ...
      try {
        var res = await ClawForgeAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          ClawForgeToast.success('Agent "' + t.name + '" spawned');
          await Alpine.store('app').refreshAgents();
