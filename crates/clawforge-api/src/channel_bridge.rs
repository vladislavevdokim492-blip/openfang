//! Channel bridge wiring — connects the ClawForge kernel to channel adapters.
//!
//! Implements `ChannelBridgeHandle` on `ClawForgeKernel` and provides the
//! `start_channel_bridge()` entry point called by the daemon.

use clawforge_channels::bridge::{BridgeManager, ChannelBridgeHandle};
use clawforge_channels::discord::DiscordAdapter;
use clawforge_channels::email::EmailAdapter;
use clawforge_channels::google_chat::GoogleChatAdapter;
use clawforge_channels::irc::IrcAdapter;
use clawforge_channels::matrix::MatrixAdapter;
use clawforge_channels::mattermost::MattermostAdapter;
use clawforge_channels::rocketchat::RocketChatAdapter;
use clawforge_channels::router::AgentRouter;
use clawforge_channels::signal::SignalAdapter;
use clawforge_channels::slack::SlackAdapter;
use clawforge_channels::teams::TeamsAdapter;
use clawforge_channels::telegram::TelegramAdapter;
use clawforge_channels::twitch::TwitchAdapter;
use clawforge_channels::types::ChannelAdapter;
use clawforge_channels::whatsapp::WhatsAppAdapter;
use clawforge_channels::xmpp::XmppAdapter;
use clawforge_channels::zulip::ZulipAdapter;
// Wave 3
use clawforge_channels::bluesky::BlueskyAdapter;
use clawforge_channels::feishu::FeishuAdapter;
use clawforge_channels::line::LineAdapter;
use clawforge_channels::mastodon::MastodonAdapter;
use clawforge_channels::messenger::MessengerAdapter;
use clawforge_channels::reddit::RedditAdapter;
use clawforge_channels::revolt::RevoltAdapter;
use clawforge_channels::viber::ViberAdapter;
// Wave 4
use clawforge_channels::flock::FlockAdapter;
use clawforge_channels::guilded::GuildedAdapter;
use clawforge_channels::keybase::KeybaseAdapter;
use clawforge_channels::nextcloud::NextcloudAdapter;
use clawforge_channels::nostr::NostrAdapter;
use clawforge_channels::pumble::PumbleAdapter;
use clawforge_channels::threema::ThreemaAdapter;
use clawforge_channels::twist::TwistAdapter;
use clawforge_channels::webex::WebexAdapter;
// Wave 5
use async_trait::async_trait;
use clawforge_channels::dingtalk::DingTalkAdapter;
use clawforge_channels::discourse::DiscourseAdapter;
use clawforge_channels::gitter::GitterAdapter;
use clawforge_channels::gotify::GotifyAdapter;
use clawforge_channels::linkedin::LinkedInAdapter;
use clawforge_channels::mumble::MumbleAdapter;
use clawforge_channels::ntfy::NtfyAdapter;
use clawforge_channels::webhook::WebhookAdapter;
use clawforge_kernel::ClawForgeKernel;
use clawforge_types::agent::AgentId;
use std::sync::Arc;
use std::time::{Duration, Instant};
use tracing::{error, info, warn};

/// Wraps `ClawForgeKernel` to implement `ChannelBridgeHandle`.
pub struct KernelBridgeAdapter {
    kernel: Arc<ClawForgeKernel>,
    started_at: Instant,
}
...
impl ChannelBridgeHandle for KernelBridgeAdapter {
    ...
    async fn spawn_agent_by_name(&self, manifest_name: &str) -> Result<AgentId, String> {
        // Look for manifest at ~/.clawforge/agents/{name}/agent.toml
        ...
        let manifest: clawforge_types::agent::AgentManifest =
    ...
    async fn uptime_info(&self) -> String {
        ...
        if hours > 0 {
            format!(
                "ClawForge status: {}h {}m uptime, {} agent(s)",
                hours,
                mins,
                agents.len()
            )
            ...
            format!(
                "ClawForge status: {}m uptime, {} agent(s)",
                mins,
                agents.len()
            )
        }
    ...
    async fn list_providers_text(&self) -> String {
        ...
        for p in catalog.list_providers() {
            let status = match p.auth_status {
                clawforge_types::model_catalog::AuthStatus::Configured => "configured",
                clawforge_types::model_catalog::AuthStatus::Missing => "not configured",
                clawforge_types::model_catalog::AuthStatus::NotRequired => "local (no key needed)",
            };
            msg.push_str(&format!(
                "  {} — {} [{}, {} model(s)]\n",
    ...
    async fn list_skills_text(&self) -> String {
        ...
        if skills.is_empty() {
            return "No skills installed. Place skills in ~/.clawforge/skills/ or install from the marketplace.".to_string();
        }
        ...
        let result = self
            ...
            .execute_run(
                ...
                |step_agent| match step_agent {
                    clawforge_kernel::workflow::StepAgent::ById { id } => {
                        let aid: AgentId = id.parse().ok()?;
                        let entry = registry_ref.get(aid)?;
                        Some((aid, entry.name.clone()))
                    }
                    clawforge_kernel::workflow::StepAgent::ByName { name } => {
    ...
    async fn list_schedules_text(&self) -> String {
        ...
        for job in &jobs {
            ...
            let sched = match &job.schedule {
                clawforge_types::scheduler::CronSchedule::Cron { expr, .. } => expr.clone(),
                clawforge_types::scheduler::CronSchedule::Every { every_secs } => {
                    format!("every {every_secs}s")
                }
                clawforge_types::scheduler::CronSchedule::At { at } => {
    ...
    async fn manage_schedule_text(&self, action: &str, args: &[String]) -> String {
        match action {
            "add" => {
                ...
                let job = clawforge_types::scheduler::CronJob {
                    id: clawforge_types::scheduler::CronJobId::new(),
                    agent_id: agent.id,
                    name: format!("chat-{}", &agent.name),
                    enabled: true,
                    schedule: clawforge_types::scheduler::CronSchedule::Cron {
                        expr: cron_expr.clone(),
                        tz: None,
                    },
                    action: clawforge_types::scheduler::CronAction::AgentTurn {
                        message: message.clone(),
                        model_override: None,
                        timeout_secs: None,
                    },
                    delivery: clawforge_types::scheduler::CronDelivery::None,
    ...
    async fn channel_overrides(
        &self,
        channel_type: &str,
    ) -> Option<clawforge_types::config::ChannelOverrides> {
    ...
    async fn authorize_channel_user(
        ...
        let auth_action = match action {
            "chat" => clawforge_kernel::auth::Action::ChatWithAgent,
            "spawn" => clawforge_kernel::auth::Action::SpawnAgent,
            "kill" => clawforge_kernel::auth::Action::KillAgent,
            "install_skill" => clawforge_kernel::auth::Action::InstallSkill,
            _ => clawforge_kernel::auth::Action::ChatWithAgent,
        };
    ...
    async fn record_delivery(
        ...
        let receipt = if success {
            clawforge_kernel::DeliveryTracker::sent_receipt(channel, recipient)
        } else {
            clawforge_kernel::DeliveryTracker::failed_receipt(
                channel,
                recipient,
    ...
    async fn a2a_agents_text(&self) -> String {
        ...
        for (url, card) in agents.iter() {
            ...
            if !desc.is_empty() {
                let short = clawforge_types::truncate_str(desc, 60);
...
fn parse_trigger_pattern(s: &str) -> Option<clawforge_kernel::triggers::TriggerPattern> {
    use clawforge_kernel::triggers::TriggerPattern;
...
pub async fn start_channel_bridge(kernel: Arc<ClawForgeKernel>) -> Option<BridgeManager> {
...
pub async fn reload_channels_from_disk(
    ...
    let fresh_config = clawforge_kernel::config::load_config(Some(&config_path));
