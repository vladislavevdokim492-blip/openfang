//! Route handlers for the ClawForge API.
...
use axum::Json;
use dashmap::DashMap;
use clawforge_kernel::triggers::{TriggerId, TriggerPattern};
use clawforge_kernel::workflow::{
    ErrorMode, StepAgent, StepMode, Workflow, WorkflowId, WorkflowStep,
};
use clawforge_kernel::ClawForgeKernel;
use clawforge_runtime::kernel_handle::KernelHandle;
use clawforge_runtime::tool_runner::builtin_tool_definitions;
use clawforge_types::agent::{AgentId, AgentIdentity, AgentManifest};
...
pub struct AppState {
    pub kernel: Arc<ClawForgeKernel>,
    pub started_at: Instant,
    /// Optional peer registry for OFP mesh networking status.
    pub peer_registry: Option<Arc<clawforge_wire::registry::PeerRegistry>>,
    /// Channel bridge manager — held behind a Mutex so it can be swapped on hot-reload.
    pub bridge_manager: tokio::sync::Mutex<Option<clawforge_channels::bridge::BridgeManager>>,
    /// Live channel config — updated on every hot-reload so list_channels() reflects reality.
    pub channels_config: tokio::sync::RwLock<clawforge_types::config::ChannelsConfig>,
        ...
        match state.kernel.verify_signed_manifest(signed_json) {
            ...
            Err(e) => {
                ...
                state.kernel.audit_log.record(
                    "system",
                    clawforge_runtime::audit::AuditAction::AuthAttempt,
...
pub async fn list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let agents: Vec<serde_json::Value> = state
        ...
        .map(|e| {
            ...
            let ready = matches!(e.state, clawforge_types::agent::AgentState::Running)
                && auth_status != "missing";
...
) -> Vec<clawforge_types::message::ContentBlock> {
    use base64::Engine;

    let upload_dir = std::env::temp_dir().join("clawforge_uploads");
    ...
    for att in attachments {
        ...
        match std::fs::read(&file_path) {
            Ok(data) => {
                ...
                blocks.push(clawforge_types::message::ContentBlock::Image {
...
pub fn inject_attachments_into_session(
    kernel: &ClawForgeKernel,
    agent_id: AgentId,
    image_blocks: Vec<clawforge_types::message::ContentBlock>,
) {
    use clawforge_types::message::{Message, MessageContent, Role};
    ...
    let mut session = match kernel.memory.get_session(entry.session_id) {
        Ok(Some(s)) => s,
        _ => clawforge_memory::session::Session {
            id: entry.session_id,
            agent_id,
            messages: Vec::new(),
        ...
        Ok(Some(session)) => {
            ...
            for m in &session.messages {
                ...
                let content = match &m.content {
                    clawforge_types::message::MessageContent::Text(t) => t.clone(),
                    clawforge_types::message::MessageContent::Blocks(blocks) => {
                        let mut texts = Vec::new();
                        for b in blocks {
                            match b {
                                clawforge_types::message::ContentBlock::Text { text } => {
                                    texts.push(text.clone());
                                }
                                clawforge_types::message::ContentBlock::Image {
                                    media_type,
                                    data,
                                } => {
                                    ...
                                    let upload_dir =
                                        std::env::temp_dir().join("clawforge_uploads");
                                ...
                                clawforge_types::message::ContentBlock::ToolUse {
                                    id,
                                    name,
                                    input,
                                } => {
                                ...
                                clawforge_types::message::ContentBlock::ToolResult { .. } => {}
                                _ => {}
                            }
                        }
                        texts.join("\n")
            ...
            for m in &session.messages {
                if let clawforge_types::message::MessageContent::Blocks(blocks) = &m.content {
                    for b in blocks {
                        if let clawforge_types::message::ContentBlock::ToolResult {
                            tool_use_id,
                            content: result,
                            is_error,
                            ..
...
pub async fn shutdown(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    state.kernel.audit_log.record(
        "system",
        clawforge_runtime::audit::AuditAction::ConfigChange,
...
pub async fn list_profiles() -> impl IntoResponse {
    use clawforge_types::agent::ToolProfile;
...
pub async fn send_message_stream(
    ...
    use futures::stream;
    use clawforge_runtime::llm_driver::StreamEvent;
...
const CHANNEL_REGISTRY: &[ChannelMeta] = &[
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "user_id", label: "Bot User ID", field_type: FieldType::Text, env_var: None, required: false, placeholder: "@clawforge:matrix.org", advanced: true },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawforge_bot", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Bot Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawforge_bot", advanced: false },
            ...
            ChannelField { key: "subreddits", label: "Subreddits", field_type: FieldType::List, env_var: None, required: false, placeholder: "clawforge, rust", advanced: true },
            ChannelField { key: "default_agent", label: "Default Agent", field_type: FieldType::Text, env_var: None, required: false, placeholder: "assistant", advanced: true },
        ],
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawforge", advanced: false },
            ChannelField { key: "channels", label: "Channels", field_type: FieldType::List, env_var: None, required: false, placeholder: "#clawforge, #general", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Bot Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawforge", advanced: false },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "topic", label: "Topic", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawforge-alerts", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawforge", advanced: false },
...
fn is_channel_configured(config: &clawforge_types::config::ChannelsConfig, name: &str) -> bool {
    match name {
...
fn channel_config_values(
    config: &clawforge_types::config::ChannelsConfig,
    name: &str,
...
pub async fn configure_channel(
    ...
    let home = clawforge_kernel::config::clawforge_home();
...
async fn send_channel_test_message(channel_name: &str, target_id: &str) -> Result<(), String> {
    ...
    let test_msg = "ClawForge test message — your channel is connected!";
...
pub async fn list_templates() -> impl IntoResponse {
    let agents_dir = clawforge_kernel::config::clawforge_home().join("agents");
...
pub async fn get_agent_kv(
    ...
    let agent_id = clawforge_kernel::kernel::shared_memory_agent_id();
...
pub async fn health(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let shared_id = clawforge_types::agent::AgentId(uuid::Uuid::from_bytes([
...
///
/// Returns counters and gauges for monitoring ClawForge in production:
/// - `clawforge_agents_active` — number of active agents
/// - `clawforge_uptime_seconds` — seconds since daemon started
/// - `clawforge_tokens_total` — total tokens consumed (per agent)
/// - `clawforge_tool_calls_total` — total tool calls (per agent)
/// - `clawforge_panics_total` — supervisor panic count
/// - `clawforge_restarts_total` — supervisor restart count
pub async fn prometheus_metrics(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let mut out = String::with_capacity(2048);

    // Uptime
    let uptime = state.started_at.elapsed().as_secs();
    out.push_str("# HELP clawforge_uptime_seconds Time since daemon started.\n");
    out.push_str("# TYPE clawforge_uptime_seconds gauge\n");
    out.push_str(&format!("clawforge_uptime_seconds {uptime}\n\n"));

    // Active agents
    let agents = state.kernel.registry.list();
    let active = agents
        .iter()
        .filter(|a| matches!(a.state, clawforge_types::agent::AgentState::Running))
        .count();
    out.push_str("# HELP clawforge_agents_active Number of active agents.\n");
    out.push_str("# TYPE clawforge_agents_active gauge\n");
    out.push_str(&format!("clawforge_agents_active {active}\n"));
    out.push_str("# HELP clawforge_agents_total Total number of registered agents.\n");
    out.push_str("# TYPE clawforge_agents_total gauge\n");
    out.push_str(&format!("clawforge_agents_total {}\n\n", agents.len()));

    // Per-agent token and tool usage
    out.push_str("# HELP clawforge_tokens_total Total tokens consumed (rolling hourly window).\n");
    out.push_str("# TYPE clawforge_tokens_total gauge\n");
    out.push_str("# HELP clawforge_tool_calls_total Total tool calls (rolling hourly window).\n");
    out.push_str("# TYPE clawforge_tool_calls_total gauge\n");
    for agent in &agents {
        let name = &agent.name;
        ...
        if let Some((tokens, tools)) = state.kernel.scheduler.get_usage(agent.id) {
            out.push_str(&format!(
                "clawforge_tokens_total{{agent=\"{name}\",provider=\"{provider}\",model=\"{model}\"}} {tokens}\n"
            ));
            out.push_str(&format!(
                "clawforge_tool_calls_total{{agent=\"{name}\"}} {tools}\n"
            ));
        }
    }
    out.push('\n');
    ...
    // Supervisor health
    let health = state.kernel.supervisor.health();
    out.push_str("# HELP clawforge_panics_total Total supervisor panics since start.\n");
    out.push_str("# TYPE clawforge_panics_total counter\n");
    out.push_str(&format!("clawforge_panics_total {}\n", health.panic_count));
    out.push_str("# HELP clawforge_restarts_total Total supervisor restarts since start.\n");
    out.push_str("# TYPE clawforge_restarts_total counter\n");
    out.push_str(&format!(
        "clawforge_restarts_total {}\n\n",
        health.restart_count
    ));

    // Version info
    out.push_str("# HELP clawforge_info ClawForge version and build info.\n");
    out.push_str("# TYPE clawforge_info gauge\n");
    out.push_str(&format!(
        "clawforge_info{{version=\"{}\"}} 1\n",
    ...
    let skills: Vec<serde_json::Value> = registry
        ...
        .map(|s| {
            let source = match &s.manifest.source {
                Some(clawforge_skills::SkillSource::ClawHub { slug, version }) => {
                    serde_json::json!({"type": "clawhub", "slug": slug, "version": version})
                }
                Some(clawforge_skills::SkillSource::OpenClaw) => {
                    serde_json::json!({"type": "openclaw"})
                }
                Some(clawforge_skills::SkillSource::Bundled) => {
                    serde_json::json!({"type": "bundled"})
                }
                Some(clawforge_skills::SkillSource::Native) | None => {
...
pub async fn install_skill(
    ...
    let skills_dir = state.kernel.config.home_dir.join("skills");
    let config = clawforge_skills::marketplace::MarketplaceConfig::default();
    let client = clawforge_skills::marketplace::MarketplaceClient::new(config);
...
pub async fn clawhub_search(
    ...
    let client = clawforge_skills::clawhub::ClawHubClient::new(cache_dir);
...
pub async fn clawhub_browse(
    ...
    let sort = match params.get("sort").map(|s| s.as_str()) {
        Some("downloads") => clawforge_skills::clawhub::ClawHubSort::Downloads,
        Some("stars") => clawforge_skills::clawhub::ClawHubSort::Stars,
        Some("updated") => clawforge_skills::clawhub::ClawHubSort::Updated,
        Some("rating") => clawforge_skills::clawhub::ClawHubSort::Rating,
        _ => clawforge_skills::clawhub::ClawHubSort::Trending,
    };
...
fn clawhub_browse_entry_to_json(
    entry: &clawforge_skills::clawhub::ClawHubBrowseEntry,
) -> serde_json::Value {
    let version = clawforge_skills::clawhub::ClawHubClient::entry_version(entry);
    serde_json::json!({
        "slug": entry.slug,
...
pub async fn activate_hand(
    State(state): State<Arc<AppState>>,
    Path(hand_id): Path<String>,
    body: Option<Json<clawforge_hands::ActivateHandRequest>>,
) -> impl IntoResponse {
            ...
            if let Some(agent_id) = instance.agent_id {
                ...
                if let Some(entry) = entry {
                    if !matches!(
                        ...
                        clawforge_types::agent::ScheduleMode::Reactive
                    ) {
...
pub async fn hand_instance_browser(
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            clawforge_runtime::browser::BrowserCommand::ReadPage,
        )
        .await
    {
        Ok(resp) if resp.success => {
            if let Some(data) = &resp.data {
                ...
                if content.len() > 2000 {
                    content = format!("{}... (truncated)", clawforge_types::truncate_str(&content, 2000));
                }
            }
        }
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            clawforge_runtime::browser::BrowserCommand::Screenshot,
        )
        .await
    {
        Ok(resp) if resp.success => {
...
pub async fn update_budget(
    ...
    let config_ptr = &state.kernel.config as *const clawforge_types::config::KernelConfig
        as *mut clawforge_types::config::KernelConfig;
...
pub async fn agent_budget_status(
    ...
    let usage_store = clawforge_memory::usage::UsageStore::new(state.kernel.memory.usage_conn());
...
pub async fn delete_session(
    ...
    let session_id = match id.parse::<uuid::Uuid>() {
        Ok(u) => clawforge_types::agent::SessionId(u),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub async fn set_session_label(
    ...
    if let Some(lbl) = label {
        if let Err(e) = clawforge_types::agent::SessionLabel::new(lbl) {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": e.to_string()})),
            );
...
pub async fn find_session_by_label(
    ...
    let agent_id = match agent_id_str.parse::<uuid::Uuid>() {
        Ok(u) => clawforge_types::agent::AgentId(u),
        Err(_) => {
            // Try name lookup
...
pub async fn migrate_detect() -> impl IntoResponse {
    match clawforge_migrate::openclaw::detect_openclaw_home() {
        Some(path) => {
            let scan = clawforge_migrate::openclaw::scan_openclaw_workspace(&path);
            (
                StatusCode::OK,
                Json(serde_json::json!({
                    "detected": true,
...
pub async fn run_migrate(Json(req): Json<MigrateRequest>) -> impl IntoResponse {
    let source = match req.source.as_str() {
        "openclaw" => clawforge_migrate::MigrateSource::OpenClaw,
        "langchain" => clawforge_migrate::MigrateSource::LangChain,
        "autogpt" => clawforge_migrate::MigrateSource::AutoGpt,
        other => {
            return (
                StatusCode::BAD_REQUEST,
                Json(
    ...
    let models: Vec<serde_json::Value> = catalog
        ...
        .filter(|m| {
            ...
            if available_only {
                ...
                if let Some(p) = provider {
                    if p.auth_status == clawforge_types::model_catalog::AuthStatus::Missing {
                        return false;
                    }
                }
            }
        ...
        .map(|m| {
            ...
            let available = catalog
                ...
                .map(|p| p.auth_status != clawforge_types::model_catalog::AuthStatus::Missing)
                .unwrap_or(m.tier == clawforge_types::model_catalog::ModelTier::Custom);
            serde_json::json!({
                "id": m.id,
...
pub async fn list_providers(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let provider_list: Vec<clawforge_types::model_catalog::ProviderInfo> = {
        let catalog = state
            .kernel
            .model_catalog
            .read()
    ...
    for p in &provider_list {
        ...
        if !p.key_required {
            ...
            let probe = clawforge_runtime::provider_health::probe_provider(&p.id, &p.base_url).await;
...
///
/// Persists to `~/.clawforge/custom_models.json` and makes the model immediately
...
pub async fn add_custom_model(
    ...
    let entry = clawforge_types::model_catalog::ModelCatalogEntry {
        id: id.clone(),
        display_name: display,
        provider: provider.clone(),
        tier: clawforge_types::model_catalog::ModelTier::Custom,
        context_window,
...
pub async fn a2a_agent_card(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    if let Some(first) = agents.first() {
        let card = clawforge_runtime::a2a::build_agent_card(&first.manifest, &base_url);
        (
            StatusCode::OK,
        ...
        let card = serde_json::json!({
            "name": "clawforge",
            "description": "ClawForge Agent OS — no agents spawned yet",
...
pub async fn a2a_list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let cards: Vec<serde_json::Value> = agents
        .iter()
        .map(|entry| {
            let card = clawforge_runtime::a2a::build_agent_card(&entry.manifest, &base_url);
...
pub async fn a2a_send_task(
    ...
    let task = clawforge_runtime::a2a::A2aTask {
        id: task_id.clone(),
        session_id: session_id.clone(),
        status: clawforge_runtime::a2a::A2aTaskStatus::Working.into(),
        messages: vec![clawforge_runtime::a2a::A2aMessage {
            role: "user".to_string(),
            parts: vec![clawforge_runtime::a2a::A2aPart::Text {
    ...
    match state.kernel.send_message(agent.id, &message_text).await {
        Ok(result) => {
            let response_msg = clawforge_runtime::a2a::A2aMessage {
        ...
        Err(e) => {
            let error_msg = clawforge_runtime::a2a::A2aMessage {
                role: "agent".to_string(),
                parts: vec![clawforge_runtime::a2a::A2aPart::Text {
                    text: format!("Error: {e}"),
                }],
...
pub async fn a2a_discover_external(
    ...
    let client = clawforge_runtime::a2a::A2aClient::new();
...
pub async fn mcp_http(
    ...
    let mut tools = builtin_tool_definitions();
    {
        ...
        for skill_tool in registry.all_tool_definitions() {
            tools.push(clawforge_types::tool::ToolDefinition {
    ...
    if method == "tools/call" {
        ...
        let kernel_handle: Arc<dyn clawforge_runtime::kernel_handle::KernelHandle> =
            state.kernel.clone() as Arc<dyn clawforge_runtime::kernel_handle::KernelHandle>;
        let result = clawforge_runtime::tool_runner::execute_tool(
            "mcp-http",
            tool_name,
            &arguments,
            Some(&kernel_handle),
    ...
    let response = clawforge_runtime::mcp_server::handle_mcp_request(&request, &tools).await;
    Json(response)
}
...
pub async fn switch_agent_session(
    ...
    let session_id = match session_id_str.parse::<uuid::Uuid>() {
        Ok(uuid) => clawforge_types::agent::SessionId(uuid),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": "Invalid session ID"})),
...
pub async fn get_agent_mcp_servers(
    ...
    if let Ok(mcp_tools) = state.kernel.mcp_tools.lock() {
        ...
        for tool in mcp_tools.iter() {
            if let Some(server) = clawforge_runtime::mcp::extract_mcp_server(&tool.name) {
...
///
/// SECURITY: Writes to `~/.clawforge/secrets.env`, sets env var in process,
...
pub async fn test_provider(
    ...
    let driver_config = clawforge_runtime::llm_driver::DriverConfig {
        provider: name.clone(),
        api_key,
    ...
    match clawforge_runtime::drivers::create_driver(&driver_config) {
        Ok(driver) => {
            // Send a minimal completion request to test connectivity
            let test_req = clawforge_runtime::llm_driver::CompletionRequest {
                model: default_model.clone(),
                messages: vec![clawforge_types::message::Message::user("Hi")],
                tools: vec![],
                max_tokens: 1,
                temperature: 0.0,
                system: None,
...
pub async fn set_provider_url(
    ...
    let probe =
        clawforge_runtime::provider_health::probe_provider(&name, &base_url).await;
...
pub async fn create_skill(
    ...
    // Write skill.toml to ~/.clawforge/skills/{name}/
    ...
    for info in registry.list_all_info() {
        ...
        let status = match &info.installed {
            ...
            Some(_) => match h.as_ref().map(|h| &h.status) {
                Some(clawforge_extensions::IntegrationStatus::Ready) => "ready",
                Some(clawforge_extensions::IntegrationStatus::Error(_)) => "error",
                _ => "installed",
            },
...
pub async fn add_integration(
    ...
    let install_err = {
        ...
        if registry.is_installed(&id) {
            ...
            let entry = clawforge_extensions::InstalledIntegration {
                id: id.clone(),
...
/// Must match the value in `clawforge-kernel/src/kernel.rs::shared_memory_agent_id()`.
...
const SCHEDULES_KEY: &str = "__clawforge_schedules";
...
pub struct PatchAgentConfigRequest {
    ...
    pub fallback_models: Option<Vec<clawforge_types::agent::FallbackModel>>,
}
...
pub async fn upload_file(
    ...
    let transcription = if content_type.starts_with("audio/") {
        let attachment = clawforge_types::media::MediaAttachment {
            media_type: clawforge_types::media::MediaType::Audio,
            mime_type: content_type.clone(),
            source: clawforge_types::media::MediaSource::FilePath {
...
pub async fn serve_upload(Path(file_id): Path<String>) -> impl IntoResponse {
    ...
    let file_path = std::env::temp_dir().join("clawforge_uploads").join(&file_id);
...
pub async fn create_approval(
    ...
    use clawforge_types::approval::{ApprovalRequest, RiskLevel};
...
pub async fn approve_request(
    ...
    match state.kernel.approval_manager.resolve(
        uuid,
        clawforge_types::approval::ApprovalDecision::Approved,
        Some("api".to_string()),
    ) {
        Ok(resp) => (
            StatusCode::OK,
...
pub async fn reject_request(
    ...
    match state.kernel.approval_manager.resolve(
        uuid,
        clawforge_types::approval::ApprovalDecision::Denied,
        Some("api".to_string()),
    ) {
        Ok(resp) => (
            StatusCode::OK,
...
pub async fn delete_cron_job(
    ...
    match uuid::Uuid::parse_str(&id) {
        Ok(uuid) => {
            let job_id = clawforge_types::scheduler::CronJobId(uuid);
...
pub async fn webhook_wake(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(body): Json<clawforge_types::webhook::WakePayload>,
) -> impl IntoResponse {
...
pub async fn webhook_agent(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(body): Json<clawforge_types::webhook::AgentHookPayload>,
) -> impl IntoResponse {
    // Check if webhook triggers are enabled
    let wh_config = match &state.kernel.config.webhook_triggers {
        Some(c) if c.enabled => c,
...
pub async fn add_binding(
    State(state): State<Arc<AppState>>,
    Json(binding): Json<clawforge_types::config::AgentBinding>,
) -> impl IntoResponse {
...
pub async fn pairing_request(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    match state.kernel.pairing.create_pairing_request() {
        Ok(req) => {
            let qr_uri = format!("clawforge://pair?token={}", req.token);
            Json(serde_json::json!({
                "token": req.token,
                "qr_uri": qr_uri,
...
pub async fn pairing_complete(
    ...
    let push_token = body
        ...
        .and_then(|v| v.as_str())
        .map(String::from);
    let device_info = clawforge_kernel::pairing::PairedDevice {
...
pub async fn copilot_oauth_start() -> impl IntoResponse {
    ...
    match clawforge_runtime::copilot_oauth::start_device_flow().await {
        Ok(resp) => {
...
pub async fn copilot_oauth_poll(
    ...
    match clawforge_runtime::copilot_oauth::poll_device_flow(&device_code).await {
        clawforge_runtime::copilot_oauth::DeviceFlowStatus::Pending => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "pending"})),
        ),
        clawforge_runtime::copilot_oauth::DeviceFlowStatus::Complete { access_token } => {
            // Save to secrets.env
            let secrets_path = state.kernel.config.home_dir.join("secrets.env");
        ...
        clawforge_runtime::copilot_oauth::DeviceFlowStatus::SlowDown { new_interval } => {
            // Update interval
        ...
        clawforge_runtime::copilot_oauth::DeviceFlowStatus::Expired => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
                Json(serde_json::json!({"status": "expired"})),
        ...
        clawforge_runtime::copilot_oauth::DeviceFlowStatus::AccessDenied => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
        ...
        clawforge_runtime::copilot_oauth::DeviceFlowStatus::Error(e) => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "error", "error": e})),
        ),
    }
...
pub async fn comms_topology(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    use clawforge_types::comms::{EdgeKind, TopoEdge, TopoNode, Topology};
    ...
    for event in &events {
        if let clawforge_types::event::EventPayload::Message(_) = &event.payload {
            if let clawforge_types::event::EventTarget::Agent(target_id) = &event.target {
...
fn filter_to_comms_event(
    event: &clawforge_types::event::Event,
    agents: &[clawforge_types::agent::AgentEntry],
) -> Option<clawforge_types::comms::CommsEvent> {
    use clawforge_types::comms::{CommsEvent, CommsEventKind};
    use clawforge_types::event::{EventPayload, EventTarget, LifecycleEvent};
    ...
    match &event.payload {
        EventPayload::Message(msg) => {
            ...
            Some(CommsEvent {
                ...
                detail: clawforge_types::truncate_str(&msg.content, 200).to_string(),
            })
        }
...
fn audit_to_comms_event(
    entry: &clawforge_runtime::audit::AuditEntry,
    agents: &[clawforge_types::agent::AgentEntry],
) -> Option<clawforge_types::comms::CommsEvent> {
    use clawforge_types::comms::{CommsEvent, CommsEventKind};
        ...
        agents
            ...
            .unwrap_or_else(|| {
                if id.is_empty() || id == "system" {
                    ...
                    clawforge_types::truncate_str(id, 12).to_string()
                }
            })
    };
        ...
        "AgentMessage" => {
            ...
            let detail = if entry.detail.starts_with("tokens_in=") {
                ...
                if entry.outcome == "ok" {
                    ...
                    format!("{} in / {} out — {}", in_tok, out_tok, clawforge_types::truncate_str(&entry.outcome, 80))
                }
            } else if entry.outcome != "ok" {
                format!("{} — {}", clawforge_types::truncate_str(&entry.detail, 80), clawforge_types::truncate_str(&entry.outcome, 80))
            } else {
                clawforge_types::truncate_str(&entry.detail, 200).to_string()
            };
            (CommsEventKind::AgentMessage, detail, "user")
        }
        "AgentSpawn" => (
            CommsEventKind::AgentSpawned,
            format!("Agent spawned: {}", clawforge_types::truncate_str(&entry.detail, 100)),
            "",
        ),
        "AgentKill" => (
            CommsEventKind::AgentTerminated,
            format!("Agent killed: {}", clawforge_types::truncate_str(&entry.detail, 100)),
            "",
        ),
        _ => return None,
    };
...
pub async fn comms_events(
    ...
    let mut comms_events: Vec<clawforge_types::comms::CommsEvent> = bus_events
        .iter()
...
pub async fn comms_send(
    State(state): State<Arc<AppState>>,
    Json(req): Json<clawforge_types::comms::CommsSendRequest>,
) -> impl IntoResponse {
    // Validate from agent exists
    let from_id: clawforge_types::agent::AgentId = match req.from_agent_id.parse() {
        Ok(id) => id,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
    ...
    let to_id: clawforge_types::agent::AgentId = match req.to_agent_id.parse() {
        Ok(id) => id,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub async fn comms_task(
    State(state): State<Arc<AppState>>,
    Json(req): Json<clawforge_types::comms::CommsTaskRequest>,
) -> impl IntoResponse {
