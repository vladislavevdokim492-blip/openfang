//! ClawForge daemon server — boots the kernel and serves the HTTP API.
...
/// Daemon info written to `~/.clawforge/daemon.json` so the CLI can find us.
...
///
/// This is extracted from `run_daemon()` so that embedders (e.g. clawforge-desktop)
...
/// Start the ClawForge daemon: boot kernel + HTTP API server.
///
...
pub async fn run_daemon(
    ...
    info!("ClawForge API server listening on http://{addr}");
...
/// Check if an ClawForge daemon is actually responding at the given address.
