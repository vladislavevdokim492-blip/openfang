...
//!
//! Allows any OpenAI-compatible client library to talk to ClawForge agents.
//! The `model` field resolves to an agent (by name, UUID, or `clawforge:<name>`),
...
use clawforge_types::message::{ContentBlock, Message, MessageContent, Role, StopReason};
...
fn resolve_agent(state: &AppState, model: &str) -> Option<(AgentId, String)> {
    // 1. "clawforge:<name>" → find agent by name
    if let Some(name) = model.strip_prefix("clawforge:") {
...
pub async fn list_models(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models: Vec<ModelObject> = agents
        .iter()
        .map(|e| ModelObject {
            id: format!("clawforge:{}", e.name),
            object: "model",
            created,
            owned_by: "clawforge".to_string(),
        })
        .collect();
