//! Load & performance tests for the ClawForge API.
//!
//! Measures throughput under concurrent access: agent spawning, API endpoint
//! latency, session management, and memory usage.
//!
//! Run: cargo test -p clawforge-api --test load_test -- --nocapture

use axum::Router;
use clawforge_api::middleware;
use clawforge_api::routes::{self, AppState};
...
async fn load_metrics_sustained() {
    ...
    for _ in 0..n {
        ...
        assert!(body.contains("clawforge_agents_active"));
    }
