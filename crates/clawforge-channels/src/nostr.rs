...
impl ChannelAdapter for NostrAdapter {
    ...
    {
        let pubkey = self.derive_pubkey();
        info!("Nostr adapter starting (pubkey: {}...)", clawforge_types::truncate_str(&pubkey, 16));
                    ...
                    let should_reconnect = loop {
                        let msg = tokio::select! {
                            _ = relay_shutdown_rx.changed() => {
                                ...
                                // Send CLOSE
                                let close_msg = serde_json::json!(["CLOSE", "clawforge-sub"]);
                                let _ = write.send(
...
mod tests {
    ...
    fn test_nostr_build_subscription() {
        ...
        assert!(sub.contains("clawforge-sub"));
