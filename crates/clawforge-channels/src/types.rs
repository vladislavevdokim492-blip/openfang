...
pub struct ChannelUser {
    ...
    /// Optional mapping to an ClawForge user identity.
    pub clawforge_user: Option<String>,
}
...
// Re-export policy/format types from clawforge-types for convenience.
pub use clawforge_types::config::{DmPolicy, GroupPolicy, OutputFormat};

/// Trait that every channel adapter must implement.
///
/// A channel adapter bridges a messaging platform to the ClawForge kernel by converting
...
pub fn split_message(text: &str, max_len: usize) -> Vec<&str> {
    ...
    while !remaining.is_empty() {
        ...
        let safe_end = clawforge_types::truncate_str(remaining, max_len).len();
