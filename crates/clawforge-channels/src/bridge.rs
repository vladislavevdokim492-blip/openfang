//! Channel bridge — connects channel adapters to the ClawForge kernel.
//!
//! Defines `ChannelBridgeHandle` (implemented by clawforge-api on the kernel) and
...
use clawforge_types::config::{ChannelOverrides, DmPolicy, GroupPolicy, OutputFormat};
use std::sync::Arc;
...
///
/// Defined here to avoid circular deps (clawforge-channels can't depend on clawforge-kernel).
/// Implemented in clawforge-api on the actual kernel.
#[async_trait]
...
async fn dispatch_message(
    ...
    if router.has_broadcast(&message.sender.platform_id) {
        ...
        if !targets.is_empty() {
            ...
            match strategy {
                clawforge_types::config::BroadcastStrategy::Parallel => {
                ...
                clawforge_types::config::BroadcastStrategy::Sequential => {
                    for (name, maybe_id) in &targets {
                        if let Some(aid) = maybe_id {
    ...
    let agent_id = router.resolve(
        ...
        message.sender.clawforge_user.as_deref(),
    );
...
async fn handle_command(
    ...
    match name {
        "start" => {
            ...
            let mut msg = "Welcome to ClawForge! I connect you to AI agents.\n\nAvailable agents:\n"
                .to_string();
            if agents.is_empty() {
        ...
        "new" => {
            ...
            let agent_id = router.resolve(
                ...
                &sender.platform_id,
                sender.clawforge_user.as_deref(),
            );
            match agent_id {
                Some(aid) => handle
                    .reset_session(aid)
