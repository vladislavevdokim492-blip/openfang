...
use chrono::Utc;
use clawforge_types::agent::{AgentEntry, AgentId};
...
impl StructuredStore {
    ...
    pub fn get(&self, agent_id: AgentId, key: &str) -> ClawForgeResult<Option<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn set(
        ...
        let conn = self
            ...
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
        let blob =
            serde_json::to_vec(&value).map_err(|e| ClawForgeError::Serialization(e.to_string()))?;
    ...
    pub fn delete(&self, agent_id: AgentId, key: &str) -> ClawForgeResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
    ...
    pub fn list_kv(&self, agent_id: AgentId) -> ClawForgeResult<Vec<(String, serde_json::Value)>> {
        ...
        for row in rows {
            let (key, blob) = row.map_err(|e| ClawForgeError::Memory(e.to_string()))?;
            let value: serde_json::Value = serde_json::from_slice(&blob)
                .unwrap_or_else(|_| {
        ...
        match result {
            Ok((name, manifest_blob, state_str, created_str, session_id_str, identity_str)) => {
                ...
                let session_id = session_id_str
                    ...
                    .map(clawforge_types::agent::SessionId)
                    .unwrap_or_else(clawforge_types::agent::SessionId::new);
    ...
    pub fn load_all_agents(&self) -> ClawForgeResult<Vec<AgentEntry>> {
        ...
        for row in rows {
            ...
            let agent_id = match uuid::Uuid::parse_str(&id_str).map(clawforge_types::agent::AgentId)
            {
                Ok(id) => id,
                Err(e) => {
            ...
            let manifest: clawforge_types::agent::AgentManifest = match rmp_serde::from_slice(
                &manifest_blob,
            ) {
                Ok(m) => m,
                Err(e) => {
    ...
    pub fn list_agents(&self) -> ClawForgeResult<Vec<(String, String, String)>> {
        ...
        let mut agents = Vec::new();
        for row in rows {
            agents.push(row.map_err(|e| ClawForgeError::Memory(e.to_string()))?);
        }
        Ok(agents)
    }
}
