...
impl UsageStore {
    ...
    pub fn record(&self, record: &UsageRecord) -> ClawForgeResult<()> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn query_hourly(&self, agent_id: AgentId) -> ClawForgeResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
    ...
    pub fn query_daily(&self, agent_id: AgentId) -> ClawForgeResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
    ...
    pub fn query_monthly(&self, agent_id: AgentId) -> ClawForgeResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
    ...
    pub fn query_global_hourly(&self) -> ClawForgeResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
    ...
    pub fn query_global_monthly(&self) -> ClawForgeResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
    ...
    pub fn query_summary(&self, agent_id: Option<AgentId>) -> ClawForgeResult<UsageSummary> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
    ...
    pub fn query_by_model(&self) -> ClawForgeResult<Vec<ModelUsage>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
        ...
        for row in rows {
            results.push(row.map_err(|e| ClawForgeError::Memory(e.to_string()))?);
        }
        Ok(results)
    }

    /// Query daily usage breakdown for the last N days.
    pub fn query_daily_breakdown(&self, days: u32) -> ClawForgeResult<Vec<DailyBreakdown>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
    ...
    pub fn query_first_event_date(&self) -> ClawForgeResult<Option<String>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
    ...
    pub fn query_today_cost(&self) -> ClawForgeResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
    ...
    pub fn cleanup_old(&self, days: u32) -> ClawForgeResult<usize> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawForgeError::Internal(e.to_string()))?;
