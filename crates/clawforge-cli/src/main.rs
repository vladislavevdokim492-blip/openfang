//! ClawForge CLI — command-line interface for the ClawForge Agent OS.
//!
//! When a daemon is running (`clawforge start`), the CLI talks to it over HTTP.
...
use clawforge_api::server::read_daemon_info;
use clawforge_kernel::ClawForgeKernel;
use clawforge_types::agent::{AgentId, AgentManifest};
...
\x1b[1;36mExamples:\x1b[0m
  clawforge init                 Initialize config and data directories
  clawforge start                Start the kernel daemon
  clawforge tui                  Launch the interactive terminal dashboard
  clawforge chat                 Quick chat with the default agent
  clawforge agent new coder      Spawn a new agent from a template
  clawforge models list          Browse available LLM models
  clawforge add github           Install the GitHub integration
  clawforge doctor               Run diagnostic health checks
  clawforge channel setup        Interactive channel setup wizard
  clawforge cron list            List scheduled jobs
  clawforge uninstall            Completely remove ClawForge from your system

\x1b[1;36mQuick Start:\x1b[0m
  1. clawforge init              Set up config + API key
  2. clawforge start             Launch the daemon
  3. clawforge chat              Start chatting!

\x1b[1;36mMore:\x1b[0m
  Docs:       https://github.com/RightNow-AI/clawforge
  Dashboard:  http://127.0.0.1:4200/ (when daemon is running)";

/// ClawForge — the open-source Agent Operating System.
#[derive(Parser)]
#[command(
    name = "clawforge",
    version,
    about = "\u{1F40D} ClawForge \u{2014} Open-source Agent Operating System",
    long_about = "\u{1F40D} ClawForge \u{2014} Open-source Agent Operating System\n\n\
...
#[derive(Subcommand)]
enum Commands {
    /// Initialize ClawForge (create ~/.clawforge/ and default config).
    Init {
        /// Quick mode: no prompts, just write config + .env (for CI/scripts).
        #[arg(long)]
        quick: bool,
    },
    /// Start the ClawForge kernel daemon (API server + kernel).
    Start,
    ...
    /// Migrate from another agent framework to ClawForge.
    Migrate(MigrateArgs),
    ...
    /// Completely uninstall ClawForge from your system.
    Uninstall {
...
enum ConfigCommands {
    ...
    /// Save an API key to ~/.clawforge/.env (prompts interactively).
    SetKey {
...
/// Get the ClawForge home directory, respecting CLAWFORGE_HOME env var.
fn cli_clawforge_home() -> std::path::PathBuf {
    if let Ok(home) = std::env::var("CLAWFORGE_HOME") {
...
fn init_tracing_file() {
    let log_dir = cli_clawforge_home();
...
fn main() {
    // Load ~/.clawforge/.env into process environment (system env takes priority).
    dotenv::load_dotenv();
...
pub(crate) fn find_daemon() -> Option<String> {
    let home_dir = cli_clawforge_home();
    ...
    match resp {
        Ok(r) => {
            ...
            if status.is_server_error() {
                ui::error_with_fix(
                    ...
                    "Check daemon logs: ~/.clawforge/tui.log",
                );
            }
            body
        }
        Err(e) => {
            ...
            if msg.contains("timed out") || msg.contains("Timeout") {
                ui::error_with_fix(
                    "Request timed out",
                    "The agent may be processing a complex request. Try again, or check `clawforge status`",
                );
            } else if msg.contains("Connection refused") || msg.contains("connect") {
                ui::error_with_fix(
                    "Cannot connect to daemon",
                    "Is the daemon running? Start it with: clawforge start",
                );
            } else {
                ui::error_with_fix(
                    &format!("Daemon communication error: {msg}"),
                    "Check `clawforge status` or restart: clawforge start",
                );
            }
            std::process::exit(1);
        }
...
fn cmd_init(quick: bool) {
    ...
    let clawforge_dir = cli_clawforge_home();

    // --- Ensure directories exist ---
    if !clawforge_dir.exists() {
        std::fs::create_dir_all(&clawforge_dir).unwrap_or_else(|e| {
            ui::error_with_fix(
                &format!("Failed to create {}", clawforge_dir.display()),
                &format!("Check permissions on {}", home.display()),
            );
            eprintln!("  {e}");
            std::process::exit(1);
        });
        restrict_dir_permissions(&clawforge_dir);
    }

    for sub in ["data", "agents"] {
        let dir = clawforge_dir.join(sub);
        if !dir.exists() {
    ...
    bundled_agents::install_bundled_agents(&clawforge_dir.join("agents"));

    if quick {
        cmd_init_quick(&clawforge_dir);
    } else if !std::io::IsTerminal::is_terminal(&std::io::stdout()) {
        ui::hint("Non-interactive terminal detected — running in quick mode");
        ui::hint("For the interactive wizard, run: clawforge init (in a terminal)");
        cmd_init_quick(&clawforge_dir);
    } else {
        cmd_init_interactive(&clawforge_dir);
    }
}

/// Quick init: no prompts, auto-detect, write config + .env, print next steps.
fn cmd_init_quick(clawforge_dir: &std::path::Path) {
    ui::banner();
    ui::blank();

    let (provider, api_key_env, model) = detect_best_provider();

    write_config_if_missing(clawforge_dir, provider, model, api_key_env);

    ui::blank();
    ui::success("ClawForge initialized (quick mode)");
    ui::kv("Provider", provider);
    ui::kv("Model", model);
    ui::blank();
    ui::next_steps(&[
        "Start the daemon:  clawforge start",
        "Chat:              clawforge chat",
    ]);
}

/// Interactive 5-step onboarding wizard (ratatui TUI).
fn cmd_init_interactive(clawforge_dir: &std::path::Path) {
    ...
    match init_wizard::run() {
        InitResult::Completed {
            ...
            ui::blank();
            ui::success("ClawForge initialized!");
            ui::kv("Provider", &provider);
            ui::kv("Model", &model);
            ...
            match launch {
                LaunchChoice::Desktop => {
                    launch_desktop_app(clawforge_dir);
                }
                LaunchChoice::Dashboard => {
                    if let Some(base) = find_daemon() {
                        ...
                        ui::error("Daemon is not running. Start it with: clawforge start");
                    }
                }
                LaunchChoice::Chat => {
...
/// Launch the clawforge-desktop Tauri app, connecting to the running daemon.
fn launch_desktop_app(_clawforge_dir: &std::path::Path) {
    ...
    let desktop_bin = {
        ...
        #[cfg(windows)]
        let name = "clawforge-desktop.exe";
        #[cfg(not(windows))]
        let name = "clawforge-desktop";
    ...
    match desktop_bin {
        Some(ref path) if path.exists() => {
            ui::success("Launching ClawForge Desktop...");
                ...
                .spawn()
            {
                ...
                Err(e) => {
                    ...
                    ui::hint("Try: clawforge dashboard");
                }
            }
        }
        _ => {
            ui::error("Desktop app not found.");
            ui::hint("Install it with: cargo install clawforge-desktop");
...
fn write_config_if_missing(
    clawforge_dir: &std::path::Path,
    provider: &str,
    model: &str,
    api_key_env: &str,
) {
    let config_path = clawforge_dir.join("config.toml");
    if config_path.exists() {
        ui::check_ok(&format!("Config already exists: {}", config_path.display()));
    } else {
        let default_config = format!(
            r#"# ClawForge Agent OS configuration
# See https://github.com/RightNow-AI/clawforge for documentation

# For Docker, change to "0.0.0.0:4200" or set CLAWFORGE_LISTEN env var.
...
fn cmd_start(config: Option<PathBuf>) {
    if let Some(base) = find_daemon() {
        ui::error_with_fix(
            ...
            "Use `clawforge status` to check it, or stop it first",
        );
        std::process::exit(1);
    }
    ...
    rt.block_on(async {
        let kernel = match ClawForgeKernel::boot(config.as_deref()) {
            Ok(k) => k,
            Err(e) => {
                boot_kernel_error(&e);
                std::process::exit(1);
        ...
        ui::kv("Model", &model);
        ui::blank();
        ui::hint("Open the dashboard in your browser, or run `clawforge chat`");
        ui::hint("Press Ctrl+C to stop the daemon");
        ui::blank();

        if let Err(e) =
            clawforge_api::server::run_daemon(kernel, &listen_addr, Some(&daemon_info_path)).await
        {
        ...
        ui::blank();
        println!("  ClawForge daemon stopped.");
    });
}

/// Read the api_key from ~/.clawforge/config.toml (if any).
fn read_api_key() -> Option<String> {
    let config_path = cli_clawforge_home().join("config.toml");
    ...
    match find_daemon() {
        ...
        None => {
            ui::warn_with_fix(
                ...
                "Is it running? Check with: clawforge status",
            );
        }
    }
}
...
fn boot_kernel_error(e: &clawforge_kernel::error::KernelError) {
    let msg = e.to_string();
    if msg.contains("parse") || msg.contains("toml") || msg.contains("config") {
        ui::error_with_fix(
            "Failed to parse configuration",
            "Check your config.toml syntax: clawforge config show",
        );
    } else if msg.contains("database") || msg.contains("locked") || msg.contains("sqlite") {
        ui::error_with_fix(
            "Database error (file may be locked)",
            "Check if another ClawForge process is running: clawforge status",
        );
    } else if msg.contains("key") || msg.contains("API") || msg.contains("auth") {
        ui::error_with_fix(
            "LLM provider authentication failed",
            "Run `clawforge doctor` to check your API key configuration",
        );
    } else {
        ui::error_with_fix(
            &format!("Failed to boot kernel: {msg}"),
            "Run `clawforge doctor` to diagnose the issue",
        );
    }
}
...
fn cmd_agent_spawn(config: Option<PathBuf>, manifest_path: PathBuf) {
    if !manifest_path.exists() {
        ui::error_with_fix(
            ...
            "Use `clawforge agent new` to spawn from a template instead",
        );
        std::process::exit(1);
    }
    ...
    if let Some(base) = find_daemon() {
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                println!("  For persistent agents, use `clawforge start` first.");
            }
            Err(e) => {
    ...
    match field {
        "model" => {
            if let Some(base) = find_daemon() {
                ...
                eprintln!("No running daemon found. Start one with: clawforge start");
                std::process::exit(1);
            }
        }
        _ => {
...
fn cmd_agent_new(config: Option<PathBuf>, template_name: Option<String>) {
    ...
    if all_templates.is_empty() {
        ui::error_with_fix(
            ...
            "Run `clawforge init` to set up the agents directory",
        );
        std::process::exit(1);
    }
        ...
        Some(ref name) => match all_templates.iter().find(|t| t.name == *name) {
            ...
            None => {
                ui::error_with_fix(
                    ...
                    "Run `clawforge agent new` to see available templates",
                );
                std::process::exit(1);
            }
        },
...
fn spawn_template_agent(config: Option<PathBuf>, template: &templates::AgentTemplate) {
    if let Some(base) = find_daemon() {
        ...
        if let Some(id) = body["agent_id"].as_str() {
            ...
            ui::blank();
            ui::hint(&format!("Chat: clawforge chat {}", template.name));
        } else {
            ui::error(&format!(
                "Failed to spawn: {}",
                body["error"].as_str().unwrap_or("Unknown error")
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                ui::hint("For persistent agents, use `clawforge start` first");
            }
            Err(e) => {
...
fn cmd_status(config: Option<PathBuf>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::section("ClawForge Daemon Status");
        ui::blank();
        ...
        ui::section("ClawForge Status (in-process)");
        ui::blank();
        ...
        ui::blank();
        ui::hint("Run `clawforge start` to launch the daemon");
...
fn cmd_doctor(json: bool, repair: bool) {
    ...
    if let Some(_h) = &home {
        ...
        // --- Check 1: ClawForge directory ---
        if clawforge_dir.exists() {
            if !json {
                ui::check_ok(&format!("ClawForge directory: {}", clawforge_dir.display()));
            }
            checks.push(serde_json::json!({"check": "clawforge_dir", "status": "ok", "path": clawforge_dir.display().to_string()}));
        } else if repair {
            if !json {
                ui::check_fail("ClawForge directory not found.");
            }
            let answer = prompt_input("    Create it now? [Y/n] ");
            if answer.is_empty() || answer.starts_with('y') || answer.starts_with('Y') {
                if std::fs::create_dir_all(&clawforge_dir).is_ok() {
                    restrict_dir_permissions(&clawforge_dir);
                    for sub in ["data", "agents"] {
                        let _ = std::fs::create_dir_all(clawforge_dir.join(sub));
                    }
                    if !json {
                        ui::check_ok("Created ClawForge directory");
                    }
                    repaired = true;
                } else {
                    if !json {
            ...
            checks.push(serde_json::json!({"check": "clawforge_dir", "status": if repaired { "repaired" } else { "fail" }}));
        } else {
            if !json {
                ui::check_fail("ClawForge directory not found. Run `clawforge init` first.");
            }
            checks.push(serde_json::json!({"check": "clawforge_dir", "status": "fail"}));
            all_ok = false;
        }

        // --- Check 2: .env file exists + permissions ---
        let env_path = clawforge_dir.join(".env");
        if env_path.exists() {
            #[cfg(unix)]
            {
                use std::os::unix::fs::PermissionsExt;
            ...
            if !json {
                ui::check_warn(
                    ".env file not found (create with: clawforge config set-key <provider>)",
                );
            }
            ...
            match toml::from_str::<toml::Value>(&config_content) {
                ...
                Err(e) => {
                    if !json {
                        ...
                        ui::hint("Fix with: clawforge config edit");
                    }
...
"#
                ...
                let _ = std::fs::create_dir_all(&clawforge_dir);
        ...
        let api_listen = {
            let cfg_path = clawforge_dir.join("config.toml");
            if cfg_path.exists() {
                std::fs::read_to_string(&cfg_path)
                    .ok()
                    .and_then(|s| toml::from_str::<clawforge_types::config::KernelConfig>(&s).ok())
                    .map(|c| c.api_listen)
        ...
        if let Some(ref base) = daemon_running {
            ...
            if !json {
                ui::check_warn("Daemon not running (start with `clawforge start`)");
            }
        ...
        let daemon_json_path = clawforge_dir.join("daemon.json");
        ...
        let db_path = clawforge_dir.join("data").join("clawforge.db");
        if db_path.exists() {
        ...
        #[cfg(unix)]
        {
            if let Ok(output) = std::process::Command::new("df")
                .args(["-m", &clawforge_dir.display().to_string()])
                .output()
            {
        ...
        let agents_dir = clawforge_dir.join("agents");
    ...
    if !any_key_set {
        if !json {
            ...
            ui::blank();
            ui::hint("Or run: clawforge config set-key groq");
        }
        all_ok = false;
    }
    ...
    // --- Check 12: Config deserialization into KernelConfig ---
    {
        ...
        if config_path.exists() {
            ...
            let config_content = std::fs::read_to_string(&config_path).unwrap_or_default();
            match toml::from_str::<clawforge_types::config::KernelConfig>(&config_content) {
                Ok(cfg) => {
                    if !json {
                    ...
                    if !cfg.include.is_empty() {
                        ...
                        for inc in &cfg.include {
                            let inc_path = clawforge_dir.join(inc);
                            if inc_path.exists() {
                                if !json {
                    ...
                    if !cfg.mcp_servers.is_empty() {
                        ...
                        for server in &cfg.mcp_servers {
                            ...
                            match &server.transport {
                                clawforge_types::config::McpTransportEntry::Stdio {
                                    command,
                                    ..
                                } => {
                                    if command.is_empty() {
                                ...
                                clawforge_types::config::McpTransportEntry::Sse { url } => {
                                    if url.is_empty() {
                                        if !json {
                                            ui::check_warn(&format!(
    ...
    // --- Check 13: Skill registry health ---
    {
        ...
        let skills_dir = cli_clawforge_home().join("skills");
        let mut skill_reg = clawforge_skills::registry::SkillRegistry::new(skills_dir.clone());
        ...
        for skill in &skills {
            if let Some(ref prompt) = skill.manifest.prompt_context {
                let warnings = clawforge_skills::verify::SkillVerifier::scan_prompt_content(prompt);
                let has_critical = warnings.iter().any(|w| {
                    matches!(
                        w.severity,
                        clawforge_skills::verify::WarningSeverity::Critical
                    )
                });
                if has_critical {
    ...
    // --- Check 14: Extension registry health ---
    {
        ...
        let mut ext_registry =
            clawforge_extensions::registry::IntegrationRegistry::new(&clawforge_dir);
    ...
    if json {
        ...
        println!();
        if all_ok {
            ui::success("All checks passed! ClawForge is ready.");
            ui::hint("Start the daemon: clawforge start");
        } else if repaired {
            ui::success("Repairs applied. Re-run `clawforge doctor` to verify.");
        } else {
            ui::error("Some checks failed.");
            if !repair {
                ui::hint("Run `clawforge doctor --repair` to attempt auto-fix");
            }
        }
    }
}
        ...
        match start_daemon_background() {
            ...
            Err(e) => {
                ui::error_with_fix(
                    ...
                    "Start it manually: clawforge start",
                );
                std::process::exit(1);
            }
        }
...
fn cmd_completion(shell: clap_complete::Shell) {
    ...
    clap_complete::generate(shell, &mut cmd, "clawforge", &mut std::io::stdout());
}
...
fn require_daemon(command: &str) -> String {
    find_daemon().unwrap_or_else(|| {
        ui::error_with_fix(
            &format!("`clawforge {command}` requires a running daemon"),
            "Start the daemon: clawforge start",
        );
        ui::hint("Or try `clawforge chat` which works without a daemon");
        std::process::exit(1);
    })
}

fn boot_kernel(config: Option<PathBuf>) -> ClawForgeKernel {
    match ClawForgeKernel::boot(config.as_deref()) {
        Ok(k) => k,
        Err(e) => {
            boot_kernel_error(&e);
            std::process::exit(1);
...
fn cmd_migrate(args: MigrateArgs) {
    let source = match args.from {
        MigrateSourceArg::Openclaw => clawforge_migrate::MigrateSource::OpenClaw,
        MigrateSourceArg::Langchain => clawforge_migrate::MigrateSource::LangChain,
        MigrateSourceArg::Autogpt => clawforge_migrate::MigrateSource::AutoGpt,
    };
    ...
    let source_dir = args.source_dir.unwrap_or_else(|| {
        ...
        match source {
            clawforge_migrate::MigrateSource::OpenClaw => home.join(".openclaw"),
            clawforge_migrate::MigrateSource::LangChain => home.join(".langchain"),
            clawforge_migrate::MigrateSource::AutoGpt => home.join("Auto-GPT"),
        }
    });

    let target_dir = cli_clawforge_home();
    ...
    let options = clawforge_migrate::MigrateOptions {
        source,
        source_dir,
        target_dir,
    ...
    match clawforge_migrate::run_migration(&options) {
        Ok(report) => {
...
fn cmd_skill_install(source: &str) {
    ...
    if source_path.exists() && source_path.is_dir() {
        ...
        if !manifest_path.exists() {
            ...
            if clawforge_skills::openclaw_compat::detect_openclaw_skill(&source_path) {
                println!("Detected OpenClaw skill format. Converting...");
                match clawforge_skills::openclaw_compat::convert_openclaw_skill(&source_path) {
                    Ok(manifest) => {
                        let dest = skills_dir.join(&manifest.skill.name);
                        // Copy skill directory
                        copy_dir_recursive(&source_path, &dest);
                        if let Err(e) = clawforge_skills::openclaw_compat::write_clawforge_manifest(
                            &dest, &manifest,
                        ) {
        ...
        let manifest: clawforge_skills::SkillManifest =
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let client = clawforge_skills::marketplace::MarketplaceClient::new(
            clawforge_skills::marketplace::MarketplaceConfig::default(),
        );
...
fn cmd_skill_list() {
    ...
    let mut registry = clawforge_skills::registry::SkillRegistry::new(skills_dir);
...
"#
    ...
    println!("  2. Test locally: clawforge skill test");
    println!(
        "  3. Install: clawforge skill install {}",
        skill_dir.display()
    );
}
...
fn cmd_channel_list() {
    ...
    if !config_path.exists() {
        println!("No configuration found. Run `clawforge init` first.");
        return;
    }
    ...
    println!("\nUse `clawforge channel setup <channel>` to configure a channel.");
}
    ...
    match channel.as_str() {
        "telegram" => {
            ...
            match dotenv::save_env_key("TELEGRAM_BOT_TOKEN", &token) {
                Ok(()) => ui::success("Token saved to ~/.clawforge/.env"),
            ...
            for (key, val) in [
                ...
                if !val.is_empty() {
                    match dotenv::save_env_key(key, val) {
                        Ok(()) => ui::success(&format!("{key} saved to ~/.clawforge/.env")),
        ...
        "email" => {
            ...
            if !password.is_empty() {
                match dotenv::save_env_key("EMAIL_PASSWORD", &password) {
                    Ok(()) => ui::success("Password saved to ~/.clawforge/.env"),
                    Err(_) => println!("    export EMAIL_PASSWORD=your_app_password"),
                }
            } else {
                ui::hint("Set later: clawforge config set-key email (or export EMAIL_PASSWORD=...)");
            }
        ...
        "signal" => {
            ...
            if !phone.is_empty() {
                match dotenv::save_env_key("SIGNAL_PHONE", &phone) {
                    Ok(()) => ui::success("Phone saved to ~/.clawforge/.env"),
        ...
        "matrix" => {
            ...
            println!("     (e.g., register @clawforge-bot:matrix.org)");
            println!("  2. Obtain an access token:");
            println!("     curl -X POST https://matrix.org/_matrix/client/r0/login \\");
            println!("       -d '{{\"type\":\"m.login.password\",\"user\":\"clawforge-bot\",\"password\":\"...\"}}'");
...
fn maybe_write_channel_config(channel: &str, config_block: &str) {
    ...
    if !config_path.exists() {
        ui::hint("No config.toml found. Run `clawforge init` first.");
        return;
    }
...
fn cmd_channel_test(channel: &str) {
    if let Some(base) = find_daemon() {
        ...
        eprintln!("Channel test requires a running daemon. Start with: clawforge start");
        std::process::exit(1);
    }
}
...
fn cmd_channel_toggle(channel: &str, enable: bool) {
    ...
    if let Some(base) = find_daemon() {
        ...
        println!("Edit ~/.clawforge/config.toml to persist this change.");
    }
}
...
fn cmd_hand_install(path: &str) {
    ...
    println!("Use `clawforge hand activate {}` to start it.", body["id"].as_str().unwrap_or("?"));
}
...
fn cmd_hand_list() {
    ...
    if let Some(arr) = Some(&arr_val) {
        ...
        println!("\nUse `clawforge hand activate <id>` to activate a hand.");
    }
}
...
/// Spawn `clawforge start` as a detached background process.
///
...
fn cmd_config_show() {
    ...
    if !config_path.exists() {
        ...
        println!("Run `clawforge init` to create one.");
        return;
    }
...
fn cmd_config_get(key: &str) {
    ...
    if !config_path.exists() {
        ui::error_with_fix("No config file found", "Run `clawforge init` first");
        std::process::exit(1);
    }
    ...
    let table: toml::Value = toml::from_str(&content).unwrap_or_else(|e| {
        ui::error_with_fix(
            ...
            "Fix your config.toml syntax, or run `clawforge config edit`",
        );
        std::process::exit(1);
    });
...
fn cmd_config_set_key(provider: &str) {
    ...
    match dotenv::save_env_key(&env_var, &key) {
        Ok(()) => {
            ui::success(&format!("Saved {env_var} to ~/.clawforge/.env"));
            // Test the key
            print!("  Testing key... ");
...
fn cmd_config_delete_key(provider: &str) {
    ...
    match dotenv::remove_env_key(&env_var) {
        Ok(()) => ui::success(&format!("Removed {env_var} from ~/.clawforge/.env")),
        Err(e) => {
...
fn cmd_config_test_key(provider: &str) {
    ...
    if std::env::var(&env_var).is_err() {
        ...
        ui::hint(&format!("Set it: clawforge config set-key {provider}"));
        std::process::exit(1);
    }
    ...
    if test_api_key(provider, &env_var) {
        ...
        ui::hint(&format!("Update key: clawforge config set-key {provider}"));
        std::process::exit(1);
    }
}
...
pub(crate) fn clawforge_home() -> PathBuf {
    if let Ok(home) = std::env::var("CLAWFORGE_HOME") {
...
// ---------------------------------------------------------------------------
// Integration commands (clawforge add/remove/integrations)
// ---------------------------------------------------------------------------

fn cmd_integration_add(name: &str, key: Option<&str>) {
    let home = clawforge_home();
    let mut registry = clawforge_extensions::registry::IntegrationRegistry::new(&home);
    ...
    let vault = if vault_path.exists() {
        let mut v = clawforge_extensions::vault::CredentialVault::new(vault_path);
        if v.unlock().is_ok() {
            Some(v)
        } else {
            None
    ...
    let mut resolver =
        clawforge_extensions::credentials::CredentialResolver::new(vault, Some(&dotenv_path))
    ...
    match clawforge_extensions::installer::install_integration(
        &mut registry,
        &mut resolver,
        name,
        &provided_keys,
        ...
        Ok(result) => {
            match &result.status {
                clawforge_extensions::IntegrationStatus::Ready => {
                    ui::success(&result.message);
                }
                clawforge_extensions::IntegrationStatus::Setup => {
                    println!("{}", result.message.yellow());
                    println!("\nTo add credentials:");
                    for env in &template.required_env {
                        if env.is_secret {
                            println!("  clawforge vault set {}  # {}", env.name, env.help);
...
fn cmd_integration_remove(name: &str) {
    ...
    match clawforge_extensions::installer::remove_integration(&mut registry, name) {
        Ok(msg) => {
            ui::success(&msg);
            // Hot-reload daemon
            if let Some(base_url) = find_daemon() {
...
fn cmd_integrations_list(query: Option<&str>) {
    ...
    let dotenv_path = home.join(".env");
    let resolver =
        clawforge_extensions::credentials::CredentialResolver::new(None, Some(&dotenv_path));

    let entries = if let Some(q) = query {
        clawforge_extensions::installer::search_integrations(&registry, q)
    } else {
        clawforge_extensions::installer::list_integrations(&registry, &resolver)
    };
    ...
    let mut by_category: std::collections::BTreeMap<
        String,
        Vec<&clawforge_extensions::installer::IntegrationListEntry>,
    ...
    for (category, items) in &by_category {
        ...
        for item in items {
            let status_badge = match &item.status {
                clawforge_extensions::IntegrationStatus::Ready => "[Ready]".green().to_string(),
                clawforge_extensions::IntegrationStatus::Setup => "[Setup]".yellow().to_string(),
                clawforge_extensions::IntegrationStatus::Available => {
                    "[Available]".dimmed().to_string()
                }
                clawforge_extensions::IntegrationStatus::Error(msg) => {
                    format!("[Error: {msg}]").red().to_string()
                }
                clawforge_extensions::IntegrationStatus::Disabled => {
    ...
    println!("  Use `clawforge add <name>` to install an integration.");
}

// ---------------------------------------------------------------------------
// Vault commands (clawforge vault init/set/list/remove)
// ---------------------------------------------------------------------------

fn cmd_vault_init() {
    let home = clawforge_home();
    let vault_path = home.join("vault.enc");
    let mut vault = clawforge_extensions::vault::CredentialVault::new(vault_path);
...
fn cmd_vault_set(key: &str) {
    ...
    if !vault.exists() {
        ui::error("Vault not initialized. Run: clawforge vault init");
        std::process::exit(1);
    }
...
fn cmd_vault_list() {
    ...
    if !vault.exists() {
        println!("Vault not initialized. Run: clawforge vault init");
        return;
    }
...
// ---------------------------------------------------------------------------
// Scaffold commands (clawforge new skill/integration)
// ---------------------------------------------------------------------------
...
fn cmd_scaffold(kind: ScaffoldKind) {
    ...
    let result = match kind {
        ScaffoldKind::Skill => {
            clawforge_extensions::installer::scaffold_skill(&cwd.join("my-skill"))
        }
        ScaffoldKind::Integration => {
            clawforge_extensions::installer::scaffold_integration(&cwd.join("my-integration"))
        }
    };
    match result {
...
fn cmd_models_list(provider_filter: Option<&str>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        let catalog = clawforge_runtime::model_catalog::ModelCatalog::new();
...
fn pick_model() -> String {
    ...
    let mut by_provider: std::collections::BTreeMap<
        String,
        Vec<&clawforge_types::model_catalog::ModelCatalogEntry>,
    > = std::collections::BTreeMap::new();
    for m in models {
...
fn cmd_logs(lines: usize, follow: bool) {
    let log_path = cli_clawforge_home().join("tui.log");
...
fn cmd_health(json: bool) {
    match find_daemon() {
        ...
        None => {
            ...
            ui::hint("Start it with: clawforge start");
            std::process::exit(1);
        }
    }
}
...
fn cmd_devices_pair() {
    ...
    if let Some(qr) = body["qr_data"].as_str() {
        ...
        println!("  Scan this QR code with the ClawForge mobile app:");
        ui::blank();
        println!("  {qr}");
        ui::blank();
...
fn cmd_system_info(json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::section("ClawForge System Info");
        ui::blank();
        ...
        ui::kv("Version", env!("CARGO_PKG_VERSION"));
        ui::kv_warn("Daemon", "NOT RUNNING");
        ui::hint("Start with: clawforge start");
    }
}
...
fn cmd_system_version(json: bool) {
    ...
    println!("clawforge {}", env!("CARGO_PKG_VERSION"));
}
...
fn cmd_reset(confirm: bool) {
    ...
    if !clawforge_dir.exists() {
        println!(
            ...
            clawforge_dir.display()
        );
        return;
    }

    if !confirm {
        println!("  This will delete all data in {}", clawforge_dir.display());
    ...
    match std::fs::remove_dir_all(&clawforge_dir) {
        Ok(()) => ui::success(&format!("Removed {}", clawforge_dir.display())),
        Err(e) => {
            ui::error(&format!("Failed to remove {}: {e}", clawforge_dir.display()));
            std::process::exit(1);
        }
    }
}
...
fn cmd_uninstall(confirm: bool, keep_config: bool) {
    ...
    println!(
        "  {}",
        "This will completely uninstall ClawForge from your system."
            .bold()
            .red()
    );
    println!();
    if clawforge_dir.exists() {
        if keep_config {
            ...
            println!("  • Remove {}", clawforge_dir.display());
        }
    }
    ...
    if find_daemon().is_some() {
        ...
        if find_daemon().is_some() {
            if let Some(info) = read_daemon_info(&clawforge_dir) {
                force_kill_pid(info.pid);
                let _ = std::fs::remove_file(clawforge_dir.join("daemon.json"));
            }
        }
    }
    ...
    // Step 6: Remove ~/.clawforge/ data
    if clawforge_dir.exists() {
        if keep_config {
            remove_dir_except_config(&clawforge_dir);
    ...
    println!();
    ui::success("ClawForge has been uninstalled. Goodbye!");
}
...
fn remove_autostart_entries(home: &std::path::Path) {
    ...
    #[cfg(target_os = "macos")]
    {
        let plist = home.join("Library/LaunchAgents/ai.clawforge.desktop.plist");
        if plist.exists() {
            // Unload first
    ...
    #[cfg(target_os = "linux")]
    {
        let desktop_file = home.join(".config/autostart/ClawForge.desktop");
        ...
        let service_file = home.join(".config/systemd/user/clawforge.service");
        if service_file.exists() {
            let _ = std::process::Command::new("systemctl")
                .args(["--user", "disable", "--now", "clawforge.service"])
                .output();
...
/// Remove lines from shell config files that add clawforge to PATH.
#[allow(unused_variables)]
fn clean_path_entries(home: &std::path::Path, clawforge_dir: &str) {
    #[cfg(not(windows))]
    {
        let shell_files = [
            home.join(".bashrc"),
        ...
        for path in &shell_files {
            ...
            let filtered: Vec<&str> = content
                .lines()
                .filter(|line| !is_clawforge_path_line(line, clawforge_dir))
                .collect();
    ...
    #[cfg(windows)]
    {
        // Read User PATH via PowerShell, filter out clawforge entries, write back
        ...
        if let Ok(out) = output {
            if out.status.success() {
                ...
                if !current.is_empty() {
                    let dir_lower = clawforge_dir.to_lowercase();
                    let filtered: Vec<&str> = current
                        .split(';')
                        .filter(|entry| {
                            let e = entry.trim().to_lowercase();
                            !e.is_empty() && !e.contains("clawforge") && !e.contains(&dir_lower)
                        })
                        .collect();
...
/// Returns true if a shell config line is an clawforge PATH export.
/// Must match BOTH an clawforge reference AND a PATH-setting pattern.
#[cfg(any(not(windows), test))]
fn is_clawforge_path_line(line: &str, clawforge_dir: &str) -> bool {
    let lower = line.to_lowercase();
    let has_clawforge = lower.contains("clawforge") || lower.contains(&clawforge_dir.to_lowercase());
    if !has_clawforge {
        return false;
    }
...
/// Remove everything in ~/.clawforge/ except config files.
fn remove_dir_except_config(clawforge_dir: &std::path::Path) {
    let keep = ["config.toml", ".env", "secrets.env"];
    let Ok(entries) = std::fs::read_dir(clawforge_dir) else {
        return;
    };
    for entry in entries.flatten() {
...
mod tests {
    ...
    fn test_doctor_skill_registry_loads_bundled() {
        let skills_dir = std::env::temp_dir().join("clawforge-doctor-test-skills");
        let mut skill_reg = clawforge_skills::registry::SkillRegistry::new(skills_dir);
    ...
    fn test_doctor_extension_registry_loads_bundled() {
        let tmp = std::env::temp_dir().join("clawforge-doctor-test-ext");
        let _ = std::fs::create_dir_all(&tmp);
        let mut ext_reg = clawforge_extensions::registry::IntegrationRegistry::new(&tmp);
    ...
    fn test_doctor_config_deser_default() {
        ...
        let config = clawforge_types::config::KernelConfig::default();
        let toml_str = toml::to_string_pretty(&config).unwrap();
        let parsed: clawforge_types::config::KernelConfig = toml::from_str(&toml_str).unwrap();
...
"#;
        let config: clawforge_types::config::KernelConfig = toml::from_str(config_toml).unwrap();
...
"#;
        ...
        assert_eq!(
            ...
            clawforge_types::config::ExecSecurityMode::Allowlist
        );
...
"#;
        ...
        match &config.mcp_servers[0].transport {
            clawforge_types::config::McpTransportEntry::Stdio { command, args } => {
    ...
    fn test_doctor_skill_injection_scan_clean() {
        ...
        let warnings = clawforge_skills::verify::SkillVerifier::scan_prompt_content(clean_content);
    ...
    fn test_doctor_hook_event_variants() {
        ...
        use clawforge_types::agent::HookEvent;
        let events = [
    ...
    fn test_uninstall_path_line_filter() {
        use super::is_clawforge_path_line;
        let dir = "/home/user/.clawforge/bin";

        // Should match: clawforge PATH exports
        assert!(is_clawforge_path_line(
            r#"export PATH="$HOME/.clawforge/bin:$PATH""#,
            dir
        ));
        assert!(is_clawforge_path_line(
            r#"export PATH="/home/user/.clawforge/bin:$PATH""#,
            dir
        ));
        assert!(is_clawforge_path_line(
            "set -gx PATH $HOME/.clawforge/bin $PATH",
            dir
        ));
        assert!(is_clawforge_path_line(
            "fish_add_path $HOME/.clawforge/bin",
            dir
        ));
        ...
        // Should NOT match: clawforge lines that aren't PATH-related
        assert!(!is_clawforge_path_line("# clawforge config", dir));
        assert!(!is_clawforge_path_line("alias of=clawforge", dir));
    }
}
