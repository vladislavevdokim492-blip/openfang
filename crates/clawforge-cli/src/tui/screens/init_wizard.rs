...
//!
//! Launched by `clawforge init` (without `--quick`). Takes over the terminal,
...
use crate::tui::theme;
use clawforge_runtime::model_catalog::ModelCatalog;
use clawforge_types::model_catalog::ModelTier;
...
struct State {
    ...
    openclaw_scan: Option<clawforge_migrate::openclaw::ScanResult>,
    migration_report: Option<clawforge_migrate::report::MigrationReport>,
...
pub fn run() -> InitResult {
    ...
    let (migrate_tx, migrate_rx) =
        std::sync::mpsc::channel::<Result<clawforge_migrate::report::MigrationReport, String>>();
...
fn handle_migration_key(
    state: &mut State,
    code: KeyCode,
    migrate_tx: &std::sync::mpsc::Sender<Result<clawforge_migrate::report::MigrationReport, String>>,
) {
        ...
        MigrationPhase::Offer => match code {
            ...
            KeyCode::Enter => {
                ...
                if yes {
                    ...
                    let target_dir = if let Ok(h) = std::env::var("CLAWFORGE_HOME") {
                        PathBuf::from(h)
                    } else {
                        dirs::home_dir().unwrap_or_else(|| PathBuf::from(".")).join(".clawforge")
                    };
                    let tx = migrate_tx.clone();
                    std::thread::spawn(move || {
                        let options = clawforge_migrate::MigrateOptions {
                            source: clawforge_migrate::MigrateSource::OpenClaw,
                            source_dir,
                            target_dir,
                            dry_run: false,
                        };
                        let result =
                            clawforge_migrate::run_migration(&options).map_err(|e| format!("{e}"));
...
fn save_config(state: &mut State) {
    ...
    let clawforge_dir = if let Ok(h) = std::env::var("CLAWFORGE_HOME") {
        PathBuf::from(h)
    } else {
        match dirs::home_dir() {
            Some(h) => h.join(".clawforge"),
            None => {
    ...
    let _ = std::fs::create_dir_all(clawforge_dir.join("agents"));
    let _ = std::fs::create_dir_all(clawforge_dir.join("data"));
    crate::restrict_dir_permissions(&clawforge_dir);
...
/// Check if the `clawforge-desktop` binary exists next to the current exe.
...
fn draw(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    // Header: "ClawForge Init  Step X of 7"
...
fn draw_migration_done(f: &mut Frame, area: Rect, state: &State) {
    ...
    if let Some(ref error) = state.migration_error {
        ...
        use clawforge_migrate::report::ItemKind;
...
fn draw_routing_pick(f: &mut Frame, area: Rect, state: &mut State, tier: usize) {
        ...
        .enumerate()
    {
        if t == tier {
            ...
            let display = clawforge_types::truncate_str(short, 14);
            summary_spans.push(Span::styled(
                format!("{name}:{display}"),
                Style::default().fg(*c),
            ));
...
fn draw_complete(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            "  How do you want to use ClawForge?",
            Style::default()
                .fg(theme::ACCENT)
                .add_modifier(Modifier::BOLD),
        )])),
