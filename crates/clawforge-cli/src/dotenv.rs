//! Minimal `.env` file loader/saver for `~/.clawforge/.env`.
//!
...
fn dotenv_clawforge_home() -> Option<PathBuf> {
    if let Ok(home) = std::env::var("CLAWFORGE_HOME") {
        return Some(PathBuf::from(home));
    }
    dirs::home_dir().map(|h| h.join(".clawforge"))
}

/// Return the path to `~/.clawforge/.env`.
pub fn env_file_path() -> Option<PathBuf> {
    dotenv_clawforge_home().map(|h| h.join(".env"))
}

/// Load `~/.clawforge/.env` and `~/.clawforge/secrets.env` into `std::env`.
///
...
/// Return the path to `~/.clawforge/secrets.env`.
pub fn secrets_env_path() -> Option<PathBuf> {
    dotenv_clawforge_home().map(|h| h.join("secrets.env"))
}
...
/// Remove a key from `~/.clawforge/.env`.
///
...
/// List key names (without values) from `~/.clawforge/.env`.
#[allow(dead_code)]
...
fn write_env_file(path: &PathBuf, entries: &BTreeMap<String, String>) -> Result<(), String> {
    let mut content =
        String::from("# ClawForge environment — managed by `clawforge config set-key`\n");
