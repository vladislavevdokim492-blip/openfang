...
/// 2. `~/.clawforge/agents/` (installed templates)
/// 3. `CLAWFORGE_AGENTS_DIR` env var
pub fn discover_template_dirs() -> Vec<PathBuf> {
    ...
    // Installed templates (respects CLAWFORGE_HOME)
    let of_home = if let Ok(h) = std::env::var("CLAWFORGE_HOME") {
        PathBuf::from(h)
    } else if let Some(home) = dirs::home_dir() {
        home.join(".clawforge")
    } else {
        std::env::temp_dir().join(".clawforge")
    };
    {
    ...
    if let Ok(env_dir) = std::env::var("CLAWFORGE_AGENTS_DIR") {
