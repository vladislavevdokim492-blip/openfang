...
use clawforge_memory::session::Session;
use clawforge_memory::MemorySubstrate;
use clawforge_skills::registry::SkillRegistry;
use clawforge_types::agent::AgentManifest;
use clawforge_types::error::{ClawForgeError, ClawForgeResult};
use clawforge_types::memory::{Memory, MemoryFilter, MemorySource};
use clawforge_types::message::{
    ContentBlock, Message, MessageContent, Role, StopReason, TokenUsage,
};
use clawforge_types::tool::{ToolCall, ToolDefinition};
...
pub struct AgentLoopResult {
    ...
    pub directives: clawforge_types::message::ReplyDirectives,
}

/// Run the agent execution loop for a single user message.
///
/// This is the core of ClawForge: it loads session context, recalls memories,
...
pub async fn run_agent_loop(
...
) -> ClawForgeResult<AgentLoopResult> {
    ...
    if let Some(hook_reg) = hooks {
        let ctx = crate::hooks::HookContext {
            ...
            event: clawforge_types::agent::HookEvent::BeforePromptBuild,
            ...
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                if text.trim() == "NO_REPLY" || parsed_directives.silent {
                    ...
                    return Ok(AgentLoopResult {
                        ...
                        cost_usd: None,
                        silent: true,
                        directives: clawforge_types::message::ReplyDirectives {
                ...
                if let Some(hook_reg) = hooks {
                    let ctx = crate::hooks::HookContext {
                        agent_name: &manifest.name,
                        agent_id: agent_id_str.as_str(),
                        event: clawforge_types::agent::HookEvent::AgentLoopEnd,
                        data: serde_json::json!({
                            "iterations": iteration + 1,
                ...
                for tool_call in &response.tool_calls {
                    ...
                    match &verdict {
                        LoopGuardVerdict::CircuitBreak(msg) => {
                            ...
                            return Err(ClawForgeError::Internal(msg.clone()));
                        }
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: clawforge_types::agent::HookEvent::BeforeToolCall,
                            data: serde_json::json!({
                    ...
                    .await
                    {
                        ...
                        Err(_) => {
                            ...
                            clawforge_types::tool::ToolResult {
                                tool_use_id: tool_call.id.clone(),
                                content: format!(
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: clawforge_types::agent::HookEvent::AfterToolCall,
                            data: serde_json::json!({
                                "tool_name": &tool_call.name,
    ...
    Err(ClawForgeError::MaxIterationsExceeded(max_iterations))
}
...
) -> ClawForgeResult<crate::llm_driver::CompletionResponse> {
    ...
    if let (Some(provider), Some(cooldown)) = (provider, cooldown) {
        match cooldown.check(provider) {
            CooldownVerdict::Reject {
                ...
                return Err(ClawForgeError::LlmDriver(format!(
    ...
    for attempt in 0..=MAX_RETRIES {
        match driver.complete(request.clone()).await {
            ...
            Err(e) => {
                ...
                return Err(ClawForgeError::LlmDriver(user_msg));
            }
        }
    }

    Err(ClawForgeError::LlmDriver(
...
mod tests {
    ...
    use clawforge_types::tool::ToolCall;
    ...
    async fn test_empty_response_after_tool_use_returns_fallback() {
        let memory = clawforge_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = clawforge_types::agent::AgentId::new();
        let mut session = clawforge_memory::session::Session {
            id: clawforge_types::agent::SessionId::new(),
            agent_id,
            messages: Vec::new(),
