...
use clawforge_skills::registry::SkillRegistry;
use clawforge_types::taint::{TaintLabel, TaintSink, TaintedValue};
use clawforge_types::tool::{ToolDefinition, ToolResult};
...
pub async fn execute_tool(
    ...
    exec_policy: Option<&clawforge_types::config::ExecPolicy>,
    tts_engine: Option<&crate::tts::TtsEngine>,
    docker_config: Option<&clawforge_types::config::DockerSandboxConfig>,
    ...
    if let Some(kh) = kernel {
        if kh.requires_approval(tool_name) {
            ...
            let summary = format!(
                "{}: {}",
                tool_name,
                clawforge_types::truncate_str(&input_str, 200)
            );
    ...
    let result = match tool_name {
        ...
        "shell_exec" => {
            ...
            let is_full_exec = exec_policy
                .is_some_and(|p| p.mode == clawforge_types::config::ExecSecurityMode::Full);
            if !is_full_exec {
        ...
        other => {
            ...
            // Fallback 2: Skill registry tool providers
                ...
                if let Some(skill) = registry.find_tool_provider(other) {
                    ...
                    match clawforge_skills::loader::execute_skill_tool(
                        &skill.manifest,
                        &skill.path,
                        other,
                        input,
    ...
    vec![
        ...
        ToolDefinition {
            ...
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "url": { "type": "string", "description": "Base URL of the remote ClawForge/A2A-compatible agent (e.g., 'https://agent.example.com')" }
                },
                "required": ["url"]
            }),
        },
...
async fn tool_web_search_legacy(input: &serde_json::Value) -> Result<String, String> {
    ...
    let resp = client
        ...
        .query(&[("q", query)])
        .header("User-Agent", "Mozilla/5.0 (compatible; ClawForgeAgent/0.1)")
        .send()
        .await
...
fn parse_entity_type(s: &str) -> clawforge_types::memory::EntityType {
    use clawforge_types::memory::EntityType;
...
fn parse_relation_type(s: &str) -> clawforge_types::memory::RelationType {
    use clawforge_types::memory::RelationType;
    match s.to_lowercase().as_str() {
...
async fn tool_knowledge_add_entity(
    ...
    let entity = clawforge_types::memory::Entity {
...
async fn tool_knowledge_add_relation(
    ...
    let relation = clawforge_types::memory::Relation {
...
async fn tool_knowledge_query(
    ...
    let pattern = clawforge_types::memory::GraphPattern {
        source,
        relation,
        target,
        max_depth,
...
async fn tool_location_get() -> Result<String, String> {
    ...
    let resp = client
        ...
        .header("User-Agent", "ClawForge/0.1")
        .send()
        .await
...
async fn tool_media_describe(
    ...
    let attachment = clawforge_types::media::MediaAttachment {
        media_type: clawforge_types::media::MediaType::Image,
        mime_type: mime.to_string(),
        source: clawforge_types::media::MediaSource::Base64 {
...
async fn tool_image_generate(
    ...
    let model = match model_str {
        "dall-e-3" | "dalle3" | "dalle-3" => clawforge_types::media::ImageGenModel::DallE3,
        "dall-e-2" | "dalle2" | "dalle-2" => clawforge_types::media::ImageGenModel::DallE2,
        "gpt-image-1" | "gpt_image_1" => clawforge_types::media::ImageGenModel::GptImage1,
        _ => {
            return Err(format!(
    ...
    let request = clawforge_types::media::ImageGenRequest {
...
async fn tool_speech_to_text(
    ...
    use clawforge_types::media::{MediaAttachment, MediaSource, MediaType};
...
mod tests {
    ...
    async fn test_canvas_present_tool() {
        ...
        let tmp = std::env::temp_dir().join("clawforge_canvas_test");
