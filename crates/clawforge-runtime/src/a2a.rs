...
//! - `build_agent_card` — expose ClawForge agents via A2A
...
pub struct AgentCard {
    ...
    /// Skills this agent can perform (A2A skill descriptors, not ClawForge skills).
...
/// A2A skill descriptor (not an ClawForge skill — describes a capability).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub async fn discover_external_agents(
    agents: &[clawforge_types::config::ExternalAgent],
...
// ---------------------------------------------------------------------------
// A2A Server — expose ClawForge agents via A2A
// ---------------------------------------------------------------------------

/// Build an A2A Agent Card from an ClawForge agent manifest.
...
impl A2aClient {
    ...
    pub async fn discover(&self, url: &str) -> Result<AgentCard, String> {
        ...
        let response = self
            .client
            .get(&agent_json_url)
            .header("User-Agent", "ClawForge/0.1 A2A")
            .send()
            .await
...
mod tests {
    ...
    fn test_a2a_config_serde() {
        use clawforge_types::config::{A2aConfig, ExternalAgent};
