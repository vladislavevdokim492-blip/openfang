...
use clawforge_kernel::config::clawforge_home;
...
/// `~/.clawforge/agents/{name}/agent.toml`, then spawns the agent.
#[tauri::command]
pub fn import_agent_toml(
    ...
    let agent_dir = clawforge_home().join("agents").join(&agent_name);
...
///
/// Copies the selected file to `~/.clawforge/skills/` and triggers a
...
pub fn import_skill_file(
    ...
    let skills_dir = clawforge_home().join("skills");
...
/// Open the ClawForge config directory (`~/.clawforge/`) in the OS file manager.
#[tauri::command]
pub fn open_config_dir() -> Result<(), String> {
    let dir = clawforge_home();
    std::fs::create_dir_all(&dir).map_err(|e| format!("Failed to create config dir: {e}"))?;
    open::that(&dir).map_err(|e| format!("Failed to open directory: {e}"))
}

/// Open the ClawForge logs directory (`~/.clawforge/logs/`) in the OS file manager.
#[tauri::command]
pub fn open_logs_dir() -> Result<(), String> {
    let dir = clawforge_home().join("logs");
