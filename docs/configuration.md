# ClawForge Configuration Reference

Complete reference for `config.toml`, covering every configurable field in the ClawForge Agent OS.
...
ClawForge reads its configuration from a single TOML file:

```
~/.clawforge/config.toml
```
...
The simplest working configuration only needs an LLM provider API key set as an environment variable. With no config file at all, ClawForge boots with Anthropic as the default provider:
...
```toml
# ============================================================
# ClawForge Agent OS -- Complete Configuration Reference
# ============================================================

# --- Top-level fields ---
home_dir = "~/.clawforge"             # ClawForge home directory
data_dir = "~/.clawforge/data"        # SQLite databases and data files
...
# --- Memory ---
[memory]
# sqlite_path = "~/.clawforge/data/clawforge.db"  # Auto-resolved if omitted
...
|-------|------|---------|-------------|
| `home_dir` | path | `~/.clawforge` | ClawForge home directory. Stores config, agents, skills. |
| `data_dir` | path | `~/.clawforge/data` | Directory for SQLite databases and persistent data. |
...
```toml
[memory]
# sqlite_path = "/custom/path/clawforge.db"
embedding_model = "all-MiniLM-L6-v2"
consolidation_threshold = 10000
decay_rate = 0.1
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `sqlite_path` | path or null | `null` | Explicit path to the SQLite database file. When `null`, defaults to `{data_dir}/clawforge.db`. |
...
Configures the OFP (ClawForge Protocol) peer-to-peer networking layer with HMAC-SHA256 mutual authentication.
...
| `user_id` | string | `""` | Bot user ID (e.g., `"@clawforge:matrix.org"`). |
...
| `nick` | string | `"clawforge"` | Bot nickname. |
| `password_env` | string or null | `null` | Env var holding the server password (optional). |
| `channels` | list of strings | `[]` | IRC channels to join (e.g., `["#clawforge", "#general"]`). |
...
| `nick` | string | `"clawforge"` | Bot nickname in Twitch chat. |
| `default_agent` | string or null | `null` | Agent name to route messages to. |
...
| `username` | string | `"clawforge"` | Bot username in Mumble. |
...
Agent-to-Agent protocol configuration, enabling inter-agent communication across ClawForge instances.
