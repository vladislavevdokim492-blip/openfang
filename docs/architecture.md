...
ClawForge is organized as a Cargo workspace with 14 crates (13 code crates + xtask). Dependencies flow downward (lower crates depend on nothing above them).

```
clawforge-cli            CLI interface, daemon auto-detect, MCP server
    |
clawforge-desktop        Tauri 2.0 desktop app (WebView + system tray)
    |
clawforge-api            REST/WS/SSE API server (Axum 0.8), 76 endpoints
    |
clawforge-kernel         Kernel: assembles all subsystems, workflow engine, RBAC, metering
    |
    +-- clawforge-runtime    Agent loop, 3 LLM drivers, 23 tools, WASM sandbox, MCP, A2A
    +-- clawforge-channels   40 channel adapters, bridge, formatter, rate limiter
    +-- clawforge-wire       OFP peer-to-peer networking with HMAC-SHA256 auth
    +-- clawforge-migrate    Migration engine (OpenClaw YAML->TOML)
    +-- clawforge-skills     60 bundled skills, FangHub marketplace, ClawHub client
    |
clawforge-memory         SQLite memory substrate, sessions, semantic search, usage tracking
    |
clawforge-types          Shared types: Agent, Capability, Event, Memory, Message, Tool, Config,
...
When `ClawForgeKernel::boot_with_config()` is called (either by the daemon or in-process by the CLI/desktop app), the following sequence executes:

```
1. Load configuration
   - Read ~/.clawforge/config.toml (or specified path)
   - Apply #[serde(default)] defaults for missing fields
   - Validate config and log warnings (missing API keys, etc.)

2. Create data directory
   - Ensure ~/.clawforge/data/ exists

3. Initialize memory substrate
   - Open SQLite database (clawforge.db)
...
The memory substrate (`clawforge-memory`) provides six layers of storage:
...
The `LlmDriver` trait (`clawforge-runtime`) provides a unified interface for all LLM providers:
...
pub trait LlmDriver: Send + Sync {
    ...
    ) -> Result<LlmResponse, ClawForgeError>;
...
The `ModelCatalog` (`clawforge-runtime/src/model_catalog.rs`) provides a registry of all known models, providers, and aliases.
...
ClawForge implements 16 security systems organized into critical fixes and state-of-the-art defenses:
...
`taint.rs` in `clawforge-types` implements taint labels and taint sets. Data from external sources carries taint labels that propagate through operations, enabling information flow analysis.
...
The channel system (`clawforge-channels`) provides 40 adapters for messaging platform integration.
...
The skill system (`clawforge-skills`) provides 60 bundled skills and supports external skill installation.
...
- **FangHub**: Native ClawForge marketplace (`FangHubClient`).
...
- **Tool Compat**: 21 OpenClaw-to-ClawForge tool name mappings in `tool_compat.rs`.
...
ClawForge implements both MCP client and server:
...
- **MCP Server** (`mcp_server.rs`): Exposes ClawForge's 23 built-in tools via the MCP protocol. Enables external tools to use ClawForge as a tool provider.
...
The ClawForge Protocol (OFP) enables peer-to-peer agent communication across machines.
...
The desktop app (`clawforge-desktop`) wraps the full ClawForge stack in a native Tauri 2.0 application.
...
+-------------------------------------------------------------------+
    ...
    v                        v                   v         v
+------------------+  +--------------+  +--------+  +-----------+
| clawforge-runtime |  | clawforge-    |  | open-  |  | clawforge- |
