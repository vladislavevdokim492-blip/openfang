...
ClawForge ships with **30 pre-built agent templates** organized into 4 performance tiers. Each template is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Templates cover software engineering, business operations, personal productivity, and everyday tasks.
...
```bash
clawforge spawn orchestrator
clawforge spawn coder
clawforge spawn --template agents/writer/agent.toml
```
...
```bash
clawforge spawn security-auditor
...
```bash
clawforge spawn data-scientist
...
```bash
clawforge spawn legal-assistant
...
> General-purpose assistant. The default ClawForge agent for everyday tasks, questions, and conversations.
...
```bash
clawforge spawn email-assistant
...
```bash
clawforge spawn customer-support
...
```bash
clawforge spawn sales-assistant
...
```bash
clawforge spawn meeting-assistant
...
```bash
clawforge spawn health-tracker
...
```bash
clawforge spawn personal-finance
...
```bash
clawforge spawn travel-planner
...
```bash
clawforge spawn home-automation
...
clawforge spawn coder --name "backend-coder"

# Spawn from a TOML file path
clawforge spawn --template agents/custom/my-agent.toml

# List running agents
clawforge agents

# Send a message
clawforge message <agent-id> "Write a function to parse TOML files"

# Kill an agent
clawforge kill <agent-id>
```
