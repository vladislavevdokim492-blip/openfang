...
cargo tauri signer generate -w ~/.tauri/clawforge.key
```
...
Your private key was saved to: ~/.tauri/clawforge.key
```
...
Open `crates/clawforge-desktop/tauri.conf.json` and replace:
...
|---|---|---|
| `TAURI_SIGNING_PRIVATE_KEY` | Contents of `~/.tauri/clawforge.key` | Yes |
...
The following icon files must exist in `crates/clawforge-desktop/icons/`:
...
## 5. Set Up the `clawforge.sh` Domain

**Status:** BLOCKING for install scripts — users run `curl -sSf https://clawforge.sh | sh`.

Options:
- **GitHub Pages**: Point `clawforge.sh` to a GitHub Pages site that redirects `/` to `scripts/install.sh` and `/install.ps1` to `scripts/install.ps1` from the repo's latest release.
- **Cloudflare Workers / Vercel**: Serve the install scripts with proper `Content-Type: text/plain` headers.
- **Raw GitHub redirect**: Use `clawforge.sh` as a CNAME to `raw.githubusercontent.com/RightNow-AI/clawforge/main/scripts/install.sh` (less reliable).

The install scripts reference:
- `https://clawforge.sh` → serves `scripts/install.sh`
- `https://clawforge.sh/install.ps1` → serves `scripts/install.ps1`

Until the domain is set up, users can install via:
```bash
curl -sSf https://raw.githubusercontent.com/RightNow-AI/clawforge/main/scripts/install.sh | sh
```
...
```bash
docker build -t clawforge:local .
docker run --rm clawforge:local --version
docker run --rm -p 4200:4200 -v clawforge-data:/data clawforge:local start
```
...
grep '"version"' crates/clawforge-desktop/tauri.conf.json
...
Visit: `https://github.com/RightNow-AI/clawforge/releases/latest/download/latest.json`
...
docker pull ghcr.io/RightNow-AI/clawforge:0.1.0

# Verify both architectures
docker run --rm ghcr.io/RightNow-AI/clawforge:latest --version
```
...
4. Open the v0.1.0 app — after 10 seconds it should:
   - Show "ClawForge Updating..." notification
...
clawforge --version  # Should print v0.1.0
