...
Skills are pluggable tool bundles that extend agent capabilities in ClawForge. A skill packages one or more tools with their implementation, letting agents do things that built-in tools do not cover. This guide covers skill creation, the manifest format, Python and WASM runtimes, publishing to FangHub, and CLI management.
...
Skills are installed to `~/.clawforge/skills/` and made available to agents through the skill registry. ClawForge ships with **60 bundled skills** that are compiled into the binary and available immediately.
...
ClawForge includes 60 expert knowledge skills compiled into the binary (no installation needed):
...
1. ClawForge sends a JSON payload to the script's stdin:
...
```python
#!/usr/bin/env python3
"""ClawForge skill: web-summarizer"""
import json
import sys
...
def summarize_url(url: str) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "ClawForge-Skill/1.0"})
...
### Using the ClawForge Python SDK

For more advanced skills, use the Python SDK (`sdk/python/clawforge_sdk.py`):

```python
#!/usr/bin/env python3
from clawforge_sdk import SkillHandler
...
```bash
clawforge skill install /path/to/my-skill
```

This reads the `skill.toml`, validates the manifest, and copies the skill to `~/.clawforge/skills/my-skill/`.

### From FangHub

```bash
clawforge skill install web-summarizer
```
...
```bash
clawforge skill install https://github.com/user/clawforge-skill-example.git
```
...
```bash
clawforge skill remove web-summarizer
```
...
FangHub is the community skill marketplace for ClawForge.
...
```bash
clawforge skill search "web scraping"
```
...
ClawForge can install and run OpenClaw-format skills. The skill installer auto-detects OpenClaw skills (by looking for `package.json` + `index.ts`/`index.js`) and converts them.

### Automatic Conversion

```bash
clawforge skill install /path/to/openclaw-skill
```

If the directory contains an OpenClaw-style skill (Node.js package), ClawForge:

1. Detects the OpenClaw format.
2. Generates a `skill.toml` manifest from `package.json`.
3. Maps tool names to ClawForge conventions.
4. Copies the skill to the ClawForge skills directory.
...
```bash
clawforge skill install /path/to/skill-directory
```

Skills imported via `clawforge migrate --from openclaw` are also scanned and reported in the migration report, with instructions for manual reinstallation.
