...
Common issues, diagnostics, and answers to frequently asked questions about ClawForge.
...
ClawForge uses `tracing` for structured logging. Set the log level via environment:

```bash
RUST_LOG=info clawforge start          # Default
RUST_LOG=debug clawforge start         # Verbose
RUST_LOG=clawforge=debug clawforge start  # Only ClawForge debug, deps at info
```
...
### `clawforge` command not found after install
...
**Common causes**:
- No API key provided: `docker run -e GROQ_API_KEY=... ghcr.io/RightNow-AI/clawforge`
...
**Fix**: Run `clawforge init` to create the default config:
```bash
clawforge init
```

This creates `~/.clawforge/config.toml` with sensible defaults.
...
```bash
RUST_LOG=clawforge_channels=debug clawforge start
```
...
**Automatic protection**: ClawForge has a built-in loop guard:
...
**Check**:
1. TOML manifest is valid: `clawforge agent spawn --dry-run manifest.toml`
...
2. Set the model to `clawforge:agent-name` (e.g., `clawforge:coder`)
...
3. Try deleting `~/.clawforge/daemon.json` and restarting
...
If slower:
- Check database size (`~/.clawforge/data/clawforge.db`)
...
1. Add the channel config to `~/.clawforge/config.toml` under `[channels]`
2. Set the required environment variables (tokens, secrets)
3. Restart the daemon

### How do I update ClawForge?

```bash
# From source
cd clawforge && git pull && cargo install --path crates/clawforge-cli
...
Only LLM API calls go to the provider's servers. All agent data, memory, sessions, and configuration are stored locally in SQLite (`~/.clawforge/data/clawforge.db`). The OFP wire protocol uses HMAC-SHA256 mutual authentication for P2P communication.

### How do I back up my data?

Back up these files:
- `~/.clawforge/config.toml` (configuration)
- `~/.clawforge/data/clawforge.db` (all agent data, memory, sessions)
- `~/.clawforge/skills/` (installed skills)

### How do I reset everything?

```bash
rm -rf ~/.clawforge
clawforge init  # Start fresh
```

### Can I run ClawForge without an internet connection?
...
### What's the difference between ClawForge and OpenClaw?

| Aspect | ClawForge | OpenClaw |
|--------|----------|----------|
...
ClawForge can import OpenClaw configs: `clawforge migrate --from openclaw`
...
```bash
RUST_LOG=clawforge_runtime=debug,clawforge_channels=info clawforge start
```

### Can I use ClawForge as a library?

Yes. Each crate is independently usable:
```toml
[dependencies]
clawforge-runtime = { path = "crates/clawforge-runtime" }
clawforge-memory = { path = "crates/clawforge-memory" }
```

The `clawforge-kernel` crate assembles everything, but you can use individual crates for custom integrations.
