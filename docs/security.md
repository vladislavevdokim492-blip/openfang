# ClawForge Security Architecture

This document provides a comprehensive technical reference for every security
system in the ClawForge Agent Operating System.  All struct names, function
...
ClawForge implements **defense-in-depth** security.  No single mechanism is
...
|---|--------|-------|------------------|
| 1 | Capability-Based Security | `clawforge-types` | Unauthorized actions by agents |
| 2 | WASM Dual Metering | `clawforge-runtime` | Infinite loops, CPU DoS |
| 3 | Merkle Audit Trail | `clawforge-runtime` | Tampered audit logs |
| 4 | Taint Tracking | `clawforge-types` | Prompt injection, data exfiltration |
| 5 | Ed25519 Manifest Signing | `clawforge-types` | Supply chain attacks |
| 6 | SSRF Protection | `clawforge-runtime` | Server-Side Request Forgery |
| 7 | Secret Zeroization | `clawforge-runtime`, `clawforge-channels` | Memory forensics, key leakage |
| 8 | OFP Mutual Auth | `clawforge-wire` | Unauthorized peer connections |
| 9 | Security Headers | `clawforge-api` | XSS, clickjacking, MIME sniffing |
| 10 | GCRA Rate Limiter | `clawforge-api` | API abuse, denial of service |
| 11 | Path Traversal Prevention | `clawforge-runtime` | Directory traversal attacks |
| 12 | Subprocess Sandbox | `clawforge-runtime` | Secret leakage via child processes |
| 13 | Prompt Injection Scanner | `clawforge-skills` | Malicious skill prompts |
| 14 | Loop Guard | `clawforge-runtime` | Stuck agent tool loops |
| 15 | Session Repair | `clawforge-runtime` | Corrupted LLM conversation history |
| 16 | Health Endpoint Redaction | `clawforge-api` | Information leakage |

---

## 2. Capability-Based Security

**Source:** `clawforge-types/src/capability.rs`

ClawForge uses capability-based security.  An agent can only perform actions
...
**Source:** `clawforge-runtime/src/sandbox.rs`
...
**Source:** `clawforge-runtime/src/audit.rs`
...
**Source:** `clawforge-types/src/taint.rs`

ClawForge implements a lattice-based taint propagation model that prevents
...
**Source:** `clawforge-types/src/manifest_signing.rs`
...
**Source:** `clawforge-runtime/src/host_functions.rs`
...
ClawForge uses `Zeroizing<String>` from the `zeroize` crate on every field
...
**LLM Drivers** (`clawforge-runtime/src/drivers/`):
...
**Channel Adapters** (`clawforge-channels/src/`):
...
**Web Search** (`clawforge-runtime/src/web_search.rs`):
...
**Embedding** (`clawforge-runtime/src/embedding.rs`):
...
**Source:** `clawforge-wire/src/peer.rs`

The ClawForge Wire Protocol (OFP) uses HMAC-SHA256 with nonce-based mutual
...
**Source:** `clawforge-api/src/middleware.rs`
...
**Source:** `clawforge-api/src/rate_limiter.rs`

ClawForge uses the Generic Cell Rate Algorithm (GCRA) for cost-aware API
...
**Source:** `clawforge-runtime/src/subprocess_sandbox.rs`
...
**Source:** `clawforge-skills/src/verify.rs`
...
**Source:** `clawforge-runtime/src/loop_guard.rs`
...
**Source:** `clawforge-runtime/src/session_repair.rs`
...
**Source:** `clawforge-api/src/routes.rs`

ClawForge provides two health endpoints with different information levels.
